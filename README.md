...
PCM 与可见识别文字。2026-09-05 的未发布 `0.2.0-candidate.10` 源码进一步修正了方向键双触发、HID 失联
后可能缺少松键、重复启动堆积，以及安装升级时旧版残留。主程序与登录自启保持普通权限；首次安装
或启用固定 HID 助手时确认一次 UAC，之后日常启动和登录自启不再反复授权。实体方向键、重新登录
自启、真实旧版升级和长期运行仍需真机复测。产物未签名；自动检查不能替代真实硬件验收。
