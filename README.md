...
PCM 与可见识别文字。2026-09-06 的未发布 `0.2.0-candidate.12` 源码保留方向键、输入
释放、单实例和旧版接管修复，并修正 Windows 规范化计划任务权限后，管理员按键
组件被误判为无效并回滚的问题。主程序与登录自启保持普通权限；首次安装或启用固定 HID
助手时确认一次 UAC，之后日常启动和登录自启不再反复授权。新配置关闭窗口时默认
隐藏到通知区域，已有合法选择保留。实体方向键、重新登录自启、真实旧版升级和长期
运行仍需真机复测。产物未签名；自动检查不能替代真实硬件验收。
...
2. 安装末尾确认一次 UAC，用于启用独立按键组件；以后直接打开“无线麦”，不需要以管理员身份运行；
