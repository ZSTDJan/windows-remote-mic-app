...
> [!IMPORTANT]
> **当前版本请临时以管理员身份启动。** 普通权限运行时仍有已知异常；在问题修复并完成验证前，请右键程序或快捷方式，选择“以管理员身份运行”。
...
PCM 与可见识别文字。2026-09-05 的未发布 `0.2.0-candidate.8` 源码进一步修正了方向键双触发、HID 失联
后可能缺少松键、重复启动堆积，以及安装升级时旧版残留。当前代码仍按主程序与登录自启保持普通
权限、仅让固定 HID 助手短时提权的方案实现，但普通权限运行仍有已知异常；问题修复并完成验证前，请临时以管理员身份启动主程序。实体方向键、重新登录自启、真实旧版升级
和长期运行仍需真机复测。产物未签名；自动检查不能替代真实硬件验收。
...
2. 每次运行时，右键程序或开始菜单中的“无线麦”，选择“以管理员身份运行”；
...
7. 按已映射为普通动作的按键验证 Windows 操作，再按已映射的语音按键验证
   ...
   关闭窗口默认会先正常停止遥控器服务并完全退出；需要后台运行时，可在“设备”页改为
   “隐藏到通知区域”。
