...
PCM 与可见识别文字。2026-09-04 的未发布源码进一步修正了方向键双触发、HID 失联
后可能缺少松键、重复启动堆积，以及安装升级时旧版残留。主程序与登录自启保持普通
权限，安装版只让固定 HID 助手按需短时提权。实体方向键、重新登录自启、真实旧版升级
...
6. 回到“设备”页点击“启动遥控器服务”，等待程序连接小米遥控器2 Pro；
...
覆盖安装新版时，安装器会先让已运行的旧版完整退出；有未保存的按键修改时，旧窗口会
出现保存或放弃提示。只有确认旧进程已经结束后才替换程序文件；无法安全退出时安装会
停止，不会强杀后继续覆盖。升级不会删除 `%LOCALAPPDATA%\RemoteMic\RC003` 下的设置、
日志或采集数据。
