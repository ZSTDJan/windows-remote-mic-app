...
PCM 与可见识别文字。2026-09-03 的未发布源码进一步修正了方向键双触发、HID 失联
后可能缺少松键、重复启动堆积和旧桥接迁移；主程序与登录自启保持普通权限，安装版
只让固定 HID 助手按需短时提权。实体方向键、重新登录自启、旧版升级迁移和长期运行
仍需真机复测。产物未签名；自动检查不能替代真实硬件验收。
