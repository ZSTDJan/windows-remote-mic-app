...
PCM 与可见识别文字。2026-09-07 的未发布 `0.2.0-candidate.23` 源码保留方向键、输入
释放、单实例、旧版接管和管理员按键助手权限修复，并补齐按键重复来源交接、快捷键录入、
输入法快捷键校验、声音通道测试恢复、搜狗语音窗口收口及通知区域窗口恢复。助手运行失败时会区分权限、目标进程、运行文件和 DLL
