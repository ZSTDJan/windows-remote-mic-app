...
状态：语音 Alt 锁死已修复并于 2026-08-30 真机收口；2026-09-03 新发现的 HID tap
失联缺少松键问题已完成源码修复，当前为检查点待实测。
...
该标记当前只由 `voice_key_physicalizer_windows.VOICE_EVENT_EXTRA_INFO` 定义，并由
...
### 2026-09-03 HID tap 失联的相邻卡键缺口

方向键双触发修正后，HID tap 成为方向自定义映射的唯一执行来源。进一步重读状态切换
发现一个具体路径：tap 在 `READY` 时已经上报某键按下，随后 Gadget 断开、租约过期或
通道失败，最后一个抬起报告便可能永远不到达；旧状态切换直接恢复 Raw Input，却没有先
释放应用层已经认领的按键，因而可能表现为方向持续移动、组合状态未结束或映射键卡住。

本批在同一个输入仲裁锁内完成顺序修正：先保持 tap 所有权，快照并清空已按 usage，逐个
送入正常松键处理，再撤销 tap 可用状态并允许 Raw Input 降级。这样最后一个 tap 报告也
不能插进“快照”和“切换”之间制造新的孤立按下。该问题有明确状态路径，不是把所有系统
卡键笼统归给无线麦；真实 RC003 断连和 WUDFHost 重启仍需人工复测。
...
- 按键检测若在语音手势中途发起，会等待下一次独立按下；本轮迟到的 HID、F5 与音频
  ...
  F5 down 门闩。
- HID tap 离开 `READY` 时先在输入仲裁锁内为全部已认领 usage 补松，再恢复 Raw Input
  降级；测试覆盖最后一个按下与状态切换并发，确认不会留下孤立 down。
- 右 Alt 物理化钩子启动时以可写 `MSG` 缓冲调用 `PeekMessageW`，确保消息队列创建过程
  使用合法指针；该钩子只处理无线麦标记的右 Alt 注入，不吞方向键或真实键盘输入。
...
- 2026-09-03 相邻 HID 失联补松、右 Alt 钩子启动和单进程接线进入最终完整检查：1889 项
  unittest 通过、7 项按平台或安全条件跳过，日志无资源泄漏告警；`compileall`、源码
  `--dry-run`、公开边界 507 个文件、`pip check` 和 `git diff --check` 通过。本批未构建。
