...
状态：第五轮迟到 HID/F5 跨轮归属缺口已修正，自动回归通过。2026-08-30 用户
反馈近期持续使用中 Alt 锁死已经几乎不再发生，说明当前修复明显有效；由于仍不是
受控复测且“几乎没有”不等于完全消失，本缺陷继续保留为检查点待实测。
...
2026-08-30 的持续使用反馈可以作为有效改善证据，但没有确认当时实际运行程序的
来源提交、EXE 哈希和完整操作顺序，不能据此把缺陷改成正式通过。当前仍标为
“检查点待实测”。至少验证：

1. HOLD 按住讲话、抬起后立即结束，键盘输入不再表现为 Ctrl/Alt/Win 卡住；
2. 快速短按且没有形成有效音频流时，抬起后下一次按键仍可正常触发；
3. 正常收到 `AudioStopped` 的会话只发送一次 KEY_UP；
4. 长按 5 至 10 秒后立即输入单个字母、点击和移动鼠标，不出现连续输入、卡键或
   短暂残留；
5. 断连或退出后仍无残留修饰键状态。
