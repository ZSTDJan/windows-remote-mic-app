...
def _dry_run() -> int:
    ...
    # Keep the explicit imports above visible to the frozen dependency scanner,
    # and exercise newly shipped modules even before another entry imports them.
    import importlib
    import pkgutil
    import ovb_rc003

    for module in pkgutil.iter_modules(ovb_rc003.__path__):
        if not module.name.startswith("__"):
            importlib.import_module(f"ovb_rc003.{module.name}")
...
def main() -> None:
    ...
    if "--observe-shortcut" in args:
        ...
        if not args or args[0] != shortcut_observation_windows.FLAG:
            raise SystemExit(2)
        raise SystemExit(shortcut_observation_windows.main(args[1:]))
    if "--list-remote-devices" in args:
        from . import remote_selection
        if not args or args[0] != remote_selection.SCAN_FLAG:
            raise SystemExit(2)
        raise SystemExit(remote_selection.scan_main(args[1:]))
