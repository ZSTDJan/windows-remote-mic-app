    ...
    *,
    ...
    try:
        ...
        try:
            ...
            pass
    _prune_stale_consumer_markers_unlocked(
        config_root,
        exclude_marker=destination,
    )
    return destination
...
def _prune_stale_consumer_markers_unlocked(
    config_root: Path,
    *,
    exclude_marker: Optional[Path] = None,
) -> None:
    directory = consumer_directory(config_root)
    try:
    ...
    for entry in entries:
        ...
        if state is _MarkerState.STALE:
            _remove_marker_best_effort(marker)
...
def remove_for_portable_consumer(
    ...
    *,
    ...
    """Remove the shared helper after explicit current-user confirmation."""

    try:
        with consumer_maintenance_lock():
            _prune_stale_consumer_markers_unlocked(config_root)
            _unregister_current_consumer_unlocked(
                config_root,
                executable=executable,
            )
