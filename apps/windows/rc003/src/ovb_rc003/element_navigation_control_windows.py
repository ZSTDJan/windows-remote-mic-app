...
class EmbeddedElementNavigationClient:
    ...
    def record_rc003_direction_edge(
        ...
        if runtime is None:
            return False
        recorder = getattr(runtime, "record_rc003_direction_edge", None)
        if not callable(recorder):
            return False
        try:
            return bool(recorder(vk, scan_code, extended, is_pressed))
        except Exception:
            return False
    ...
    def route_mapped_key(self, vk: int) -> bool:
        with self._lock:
            runtime = self._runtime
        route = getattr(runtime, "route_mapped_key", None)
        return bool(route(vk)) if callable(route) else False

    def route_local_key(self, vk: int, pressed: bool, repeat: bool, modified: bool) -> bool:
        with self._lock:
            runtime = self._runtime
        route = getattr(runtime, "route_local_key", None)
        return bool(route(vk, pressed, repeat, modified)) if callable(route) else False
...
def route_mapped_navigation_key(vk: int) -> bool:
    return _DEFAULT_CLIENT.route_mapped_key(vk)


def route_local_navigation_key(vk: int, pressed: bool, repeat: bool, modified: bool) -> bool:
    return _DEFAULT_CLIENT.route_local_key(vk, pressed, repeat, modified)
...
def record_rc003_direction_edge(
    ...
    return _DEFAULT_CLIENT.record_rc003_direction_edge(
        vk,
        scan_code,
        extended,
        is_pressed,
