...
_SETTINGS_WINDOW_RESTORE_CAPABILITY_PROPERTY = (
    "RemoteMicRC003.SettingsWindowRestoreRequestV1"
)
...
def consume_settings_window_restore_request(
    hwnd: int,
    *,
    _remove_property: RemoveWindowPropertyFn = _real_remove_window_property,
) -> bool:
    """Consume one duplicate-launch restore request inside the Qt process."""
...
def _real_activate_marked_window(
    property_name: str,
    *,
    _require_restore_request: bool = False,
) -> bool:
    _require_windows()
    user32 = ctypes.WinDLL("user32", use_last_error=True)
    enum_windows_proc = ctypes.WINFUNCTYPE(
    ...
    if _require_restore_request or user32.GetPropW(
        ...
        # Let Qt change its own visibility state. Calling ShowWindow directly
        # can make the native HWND visible while QWindow still thinks it is
        # hidden, which makes the next close-to-tray request a no-op.
        return bool(
            user32.SetPropW(
                hwnd,
                _SETTINGS_WINDOW_RESTORE_REQUEST_PROPERTY,
    ...
    # Compatibility for an older running build that does not understand the
    # in-process restore request yet.
    _SW_SHOW = 5
    _SW_RESTORE = 9
...
def _real_request_marked_window_restore(property_name: str) -> bool:
    """Require the Qt-owned restore path for an identical running copy."""

    return _real_activate_marked_window(
        property_name,
        _require_restore_request=True,
    )
...
def activate_current_runtime_settings_window(
    *,
    _activate: ActivateMarkedWindowFn = _real_request_marked_window_restore,
) -> bool:
