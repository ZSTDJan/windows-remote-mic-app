...
`src/ovb_rc003/VERSION`（当前 `0.2.0-candidate.3`），程序、冻结构建、安装器和 CI
包装都读取该文件，
...
- 程序、PyInstaller、安装器和 CI 改用同一份 `VERSION`，当前窗口标题显示
  `0.2.0-candidate.3`，避免旧安装版版本号高于新安装包时无法判断实际启动的是哪版。
...
- 相关定向测试 172 项、最终完整 unittest 1929 项通过，7 项按平台或安全条件跳过；
