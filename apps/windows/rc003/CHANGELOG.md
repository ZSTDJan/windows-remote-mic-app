...
自动改成另一种语音行为。新增的“松手后再触发一次”仅用于关闭或提交某些宿主
语音窗口，默认关闭，不会让遥控器松手后继续传声。自定义组合键也不再随长按
定时重复。本节不得扩大为当前候选已完成真机验收的声明。
维护证据与验收入口见
`MAINTENANCE.md`、
...
- **统一未保存提示**：映射、按住说话快捷键、松手收尾、输出设备、设备选择和
...
- **旧语音配置让话筒键只输入日期**：核实被测机仍保存 schema 1 开关型配置，
  ...
  不是新版按住说话链路启动后失效。程序继续拒绝自动转换旧开关型配置，错误
  改为显示“话筒键”，要求用户明确选择“按住说话”并保存启动。
...
- 配置升级到 schema 2。旧开关型、非话筒键语音和次级语音配置整键停用，要求
  用户明确重新选择；程序不会把旧开关型自动解释成按住说话。
- 新增默认关闭的“松手后再触发一次”。正常松手先释放宿主快捷键；声音停止后
  约 120 毫秒补发一次，缺少停止信号时约 800 毫秒兜底一次。声音停止先于物理
  松键时也走同一收尾，并保证不会补发两次。
...
- 本批 Qt 设置页 110 项通过；相关定向测试 356 项通过、1 项跳过；完整
  unittest 1157 项通过、7 项按平台或安全条件跳过，退出码 0。
- `compileall`、`pip check`、PowerShell parser、`git diff --check` 通过；公开
  边界扫描 279 个文件通过。新的本机 `hold2` 候选待从已提交源码构建，本批
  不制作外发 ZIP。
