...
因此 TOGGLE 已从“待复测修复”降为实验性。最后的规范
`START_SEARCH -> MIC_OPEN` 真机探针也未收到实体 `START_SEARCH`，当前
RC003 开关型持续语音已判定未通过，界面隐藏或移除方式待用户决定。
本节不得扩大为完整真机通过声明。
维护证据与验收入口见
`MAINTENANCE.md`、
...
- RC003 正式语音能力边界收敛为 HOLD；最后一条 On-request 真机探针未收到
  实体 `START_SEARCH`，TOGGLE 不再有待验证路径。配置和实现暂未擅自删除，
  等待用户决定界面隐藏还是正式移除。
- 新增隔离的 `--on-request-probe` 和便携启动脚本，只执行
  `实体 START_SEARCH -> MIC_OPEN -> 第二次按下 MIC_CLOSE` 的最后能力验证。
  它不启动 HID、宿主快捷键或音频输出，只临时吞掉遥控器泄漏的 F5，并写入
  隐私安全的控制/PCM 计数。成功必须是 `reason=MIC_OPEN, stream_id=0` 的
  会话自身持续超过一秒，普通长按 HOLD 音频不能误判。
