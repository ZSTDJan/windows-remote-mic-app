...
配置，按键页集中管理两种语音方式及各自快捷键；该界面批次不改变 `fix12`
的语音时序实现，待新候选人工复测。
本节不得扩大为完整真机通过声明。
维护证据与验收入口见
`MAINTENANCE.md`、
...
- 语音设置从连接页迁移到按键映射页，并按“按一下切换”和“按住说话”分为
  两个明确入口。两种方式分别保存自己的宿主快捷键，切换当前方式时不会覆盖
  另一套配置；麦克风映射卡只显示当前语音动作与快捷键，不再提供重复编辑项。
- 设置页保存提示区分热应用与重启边界：按键映射、语音生命周期和快捷键在
