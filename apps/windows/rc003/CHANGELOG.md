...
状态：当前源码自动检查通过，部分历史真机路径通过；2026-08-24 的
`selfcheck1` 已从当前任务工作树完成本机构建和便携 ZIP，迟到 F5 隔离、桥接
存活刷新和首次按键提示仍待真机复测。
...
### 2026-09-03 安装版方向键单次触发与预授权 HID 助手
...
- 主程序、桥接和登录自启继续使用普通权限。安装完成后单独确认一次 UAC，把不接受
...
- 当前预授权方式要求正在登录的 Windows 账号本身属于管理员组；标准账号在 UAC 中输入
  另一管理员账号凭据，不能获得当前账号可无提示调用的管理员任务。完整 unittest 共
  1880 项，8 项按平台或安全条件跳过；公开边界、语法、冻结入口和 PowerShell 解析检查
  通过。本轮不构建、不打包、不发布，安装、重登自启、实体方向键、助手恢复和卸载仍为
  检查点待实测。
