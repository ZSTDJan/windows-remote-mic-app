...
- 最终完整测试：980 项通过，7 项安全或平台相关跳过，退出码 0。
- 公开边界扫描：通过，扫描 223 个文件。
...
- 最新候选目录：源码仓库同级的
  `RemoteMicRC003-0.1.0-candidate-20260820-fix5/`。
- 候选 EXE SHA-256：
...
仍未完成：`fix5` 切换语音、托盘设置页逐键检测，以及断连重连、休眠恢复、
长期运行与杀软兼容验收。按住说话的本机基础语音链路已经通过，不再列为
完全未验证。
