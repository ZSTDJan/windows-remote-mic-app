...
- `fix7` 公开边界扫描：通过，扫描 225 个文件；`compileall` 与
  `git diff --check` 通过。
...
- 最新候选目录：源码仓库同级的
  `RemoteMicRC003-0.1.0-candidate-20260820-fix7/`。
- `fix7` 候选 EXE SHA-256：
...
仍未完成：`fix7` 切换语音，以及断连重连、休眠恢复、长期运行与杀软兼容
验收。异机多候选连接、设置页逐键检测和按住说话基础语音链路已经通过，
不再列为完全未验证。
