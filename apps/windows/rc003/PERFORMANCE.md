...
现在由桌面主进程内的元素导航运行对象加载，不再按正常产品路径启动独立伴随进程。
UI Automation 扫描与界面动作回到 Qt 事件循环执行；性能继续单独放在本文附录，
不能与 BLE、按键或音频结论混在一起。下文测量仅代表各自注明的历史基线，
不是最新源码的重新测量结果。
...
本节保留首次审计时的问题和设计理由，不是当前未修复清单。第 3 节复测与第 6 节
实施结果优先；其中“当前同步调用”“每次都会”等措辞均指该次审计基线。
接续优化前先核对现行源码，不重复实施已经完成的线程隔离、延迟加载或缓存。
...
元素导航已通过“元素导航开关”进入按键映射，当前运行在桌面主进程内。
旧伴随进程的测量不能作为当前进程隔离或性能保证。桥接线程提交请求，Qt 事件循环
承担导航扫描与界面动作；目标窗口枚举、UIA 树遍历和跨进程属性读取仍需控制耗时，
不能声称导航卡顿不会影响设置窗口。当前架构见 `WINDOWS-ARCHITECTURE-LEDGER.md`。
...
位图占用；这些建议只针对元素导航，不是 BLE、按键或音频主链的测量结论。
