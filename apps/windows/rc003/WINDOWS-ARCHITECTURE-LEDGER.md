...
- 麦克风键不仅能唤起宿主语音界面，还能把实际语音送到宿主选择的输入链；
...
2. 在设置窗口中选择 RC003、语音输出端点、语音生命周期、宿主快捷键和普通
   按键动作；
3. 保存并启动后台桥接，之后设置窗口可以关闭，桥接继续在通知区域运行；
4. 普通按钮经 HID/Raw Input 识别后执行映射动作；麦克风按钮同时驱动宿主
   快捷键和 BLE/ATVV 音频会话；
...
这里有两条相互独立但会在麦克风键处汇合的数据链：

- HID/按键链负责判断按下了哪个实体按钮，并执行 Windows 动作；
- BLE/ATVV 链负责打开遥控器麦克风、接收压缩音频、解码并播放到选定端点。

麦克风键可能同时从 ATVV、Windows F5、Raw Input 和 HID tap 到达。因此
`app.py` 有跨来源手势门闩，必须把同一次实体按下合并成一次宿主语音动作。
...
| 路径 | 内容 |
| --- | --- |
| `config.json` | 设备选择、增益、重试、语音模式/快捷键、输出端点 |
...
桥接按 mtime 热加载按键、语音模式和快捷键。活跃语音会话不会中途切换状态
