...
- 被映射为语音的按键不仅能唤起宿主语音界面，还能把实际语音送到宿主选择的
  输入链；
- “按住说话”是 RC003 已确认的正式硬件生命周期；“按一下开始、再按一下
  结束”只保留为待完成 On-request 真机探针的实验能力；
- 后台桥接可见、可退出、可重启，并能在失败时留下不泄露设备身份的诊断证据。

当前阶段是“完整源码检查完成、最新检查点待重新构建和真机复测”。基础按键与
...
2. 在设置窗口中选择 RC003 和语音输出端点，维护开关型/按住型两套语音快捷键，
   再为任一实体按键选择普通动作、开关型语音或按住型语音；
3. 保存并启动后台桥接，之后设置窗口可以关闭，桥接继续在通知区域运行；
4. 按钮经 HID/Raw Input 识别后执行主映射；普通动作进入 Windows 动作分发，
   唯一的语音主映射同时驱动对应快捷键和 BLE/ATVV 音频会话；
...
- HOLD 正式生命周期、实验性 TOGGLE 和两套宿主快捷键交付；
...
| TOGGLE | Windows 端第一次按下开始、第二次按下结束的实验状态机；RC003 松开后持续 PCM 尚未证明 |
...
- 专用语音状态机：显式 `VOICE_TOGGLE` / `VOICE_HOLD` 主动作即时处理按下与
  释放边沿，不等待双击或长按判定。

动作类型而不是实体按键名称决定分发路径。麦克风键配置为普通动作时仍可使用
单击、双击和长按；任一按键配置为语音主动作时，该行已有的普通次级设置保留
但不执行，切回普通主动作后恢复。
...
`voice_controller.py` 只决定状态，不直接操作 Windows：
...
上述 TOGGLE 条目描述当前 Windows 状态机，不代表 RC003 固件已支持松开后
持续传音。产品资料和上游真机对照都确认普通语音键是 HOLD；主动 `MIC_OPEN`
可只返回 stream 0 的开始控制事件而不发送音频。唯一尚未排除的是先收到实体
`START_SEARCH`、再按 On-request 模式回复 `MIC_OPEN` 的规范路径，详见
...
桥接发送的右 Alt injected 标记，不改变其他按键。
...
| 路径 | 内容 |
| --- | --- |
| `config.json` | 设备选择、增益、重试、两套语音快捷键、派生兼容模式/快捷键、输出端点 |
...
桥接按 mtime 热加载按键动作和两套语音快捷键。活跃语音会话不会中途切换
...
中独立维护两套快捷键；实际生命周期由 `key_bindings.json` 中唯一语音主映射
的 `VOICE_TOGGLE` / `VOICE_HOLD` 动作决定。`voice_trigger_mode` 与
`voice_hotkey` 是由该动作派生给现有桥接读取接口的兼容字段，不再是用户选择的
全局模式。旧配置只有 generic `VOICE` 与 `voice_hotkey` 时，加载器按旧
`voice_trigger_mode` 迁移为显式语音动作，并为另一模式补齐默认快捷键。
...
13 个实体按键的主映射统一可编辑，可选择普通动作、开关型语音或按住型语音；
顶部语音面板只维护两套对应快捷键，不再选择全局模式。最多允许一个语音主映射，
语音主映射行的双击与长按暂停并隐藏，原有普通次级设置继续保存；保存层仍拒绝
把语音动作本身放入次级手势。麦克风键默认映射为语音，但可以改为普通动作。
...
源码检查点 `eafd203` 已完成 1090 项 unittest，7 项安全或平台条件跳过；
`compileall`、`pip check`、PowerShell parser、公开边界扫描、QML 离屏加载和
本机 Windows API smoke 均通过。详细命令与证据见
`reviews/2026-08-21-full-code-audit.md`。这些结果证明当前源码的自动化基线，
不证明冻结包已经通过 RC003、VB-CABLE 或目标输入法的端到端实机验收。

截至 2026-08-21，历史异机结果已证明：多候选中可选出唯一可用 RC003、全部
