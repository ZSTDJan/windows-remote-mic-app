...
双击、长按和遥控器组合。tap 未能接管时，全部自定义按键映射停用，只保留 Windows
...
配置 schema 为 12。当前语音设置只写按住说话快捷键和分程序记忆；旧开关型语音
不会被解释成按住说话。桌面行为分别保存自动启动桥接和关闭窗口方式：新配置、缺失值
或非法值默认“隐藏到通知区域”，已有合法的“隐藏到通知区域”或“完全退出”选择原样
保留。运行时迁移标记不会写回用户文件，只有用户明确重新选择并保存后才生成新配置。
