...
- “按住说话”是 RC003 唯一正式实体语音生命周期；最后一条 On-request 真机探针
  未收到启动信号后，遥控器点按持续语音和专项程序已经撤下。宿主程序仍可按自身
  协议把一次实体按住翻译为成对短点；
...
- 不把保留的 DJI Mic 2 底层兼容代码扩展成另一条硬件开发主线；
...
- Quicker 不是核心运行依赖。未来只需要新增一个动作出口，不应反向侵入
  BLE、HID 或语音状态机。
- DJI Mic 2 不再提供设置或自动诊断入口；设备档案和端点识别只保留作旧配置兼容，
  不复用 RC003 桥接链路。
...
HID、合成按键或音频资源。关闭窗口默认按正常清理顺序结束全部长期资源；用户明确选择
“隐藏到通知区域”时，通知区域和 worker 才继续由同一进程持有。
...
`voice_controller.py` 只决定实体按住状态，不直接操作 Windows：话筒键按下产生
逻辑 key-down，物理松手产生逻辑 key-up；如果 Windows 没有提供松手边沿，设备的
`AUDIO_STOP` 负责同一个结束边沿兜底。搜狗等按住型程序直接接收按下和抬起；微信
输入法把开始和结束边沿各翻译为一次完整短点，并在已有音频流时等 `AUDIO_STOP`
再发送结束短点，避免物理松手提前截掉尾音。

遥控器点按持续语音实验的最后一条 On-request 真机诊断已经完成，协商成功后仍未
收到实体 `START_SEARCH`。因此该实体运行路径、设置项和专项程序均已撤下；微信的
成对短点只是目标程序触发协议，不改变用户的按住说话操作，也不重新提供遥控器
开关型语音。
...
当前实现不采用 9.5 的私有窗口消息。桥接只解析搜狗已登记的组件管理程序路径，并以
固定参数调用 `AIVoiceInputComBundle/AIVoiceInputCom --auto-launch` 预热一次；真实语音
按键后通过语音助手进程是否拥有可见顶层窗口判断就绪，失败时只再预热和检查一次，
不重复发送用户快捷键。微信输入法不使用该预热，只复用统一结果诊断。

统一结果诊断在宿主快捷键前后读取前台 PID、窗口/焦点控件类名和
`WM_GETTEXTLENGTH`，单次文本长度查询最多等待 20 ms。它不读取窗口标题或文本内容，
也不会因为焦点或长度没有变化而自动重发、粘贴或改变输入法设置。
...
当前只提供 RC003 模式，显示蓝牙配对、可选 HID tap/VB-CABLE 与宿主快捷键边界。
VB-CABLE 的检测、确认和 UAC 安装继续只有 `DiagnosticsPage.qml` 一套流程，权限页
只导航，不复制系统变更操作。自动诊断不再运行 DJI Mic 2 录音端点检查。
...
并校验窗口所属 PID，随后把剩余设置壳唤到前台，等待用户保存或放弃修改并完全退出；
安装器不强杀设置壳。只有明确权限不足时才
