# RC003 Windows 项目认知与构成原理总台账

历史认知基线：2026-08-22，完整源码检查点 `eafd203`，统一按键/语音映射
检查点 `eaabde8`，全局设置框架检查点 `addc2b0`，BLE 音频停止排序检查点
`a189a1c`，On-request 专项探针检查点 `dc82e1a`。
其中 On-request 检查点只用于追溯已完成的硬件边界调查，专项程序当前已经撤下。
当前口径校核：2026-09-12；上述提交不是最新工作区或发布包身份。
按键接管以 8.2 节为准，实际通过范围以 `TESTING.md` 各条目为准。

## 文档定位

本文是独立维护的无线麦 Windows 客户端的项目认知主入口。它依据源码
检查、现有测试、构建安装链和已经取得的真机反馈，沉淀以后接手项目时仍然
需要知道的背景与原理，主要回答：

1. 项目为什么存在，当前真正要解决什么用户问题；
2. 软件由哪些进程、线程、模块和第三方组件组成；
3. 实体按键动作与语音怎样从遥控器到达 Windows 应用；
4. 配置、日志、构建产物和运行资源分别由谁管理；
5. 哪些能力已有证据，哪些仍需要当前构建的真实硬件验证。

本文不承担逐次修改流水、单个故障排查过程或发布包哈希记录。具体故障经过见
`bugs/`，维护批次见 `MAINTENANCE.md`，发布变化见 `CHANGELOG.md`，单次完整
检查证据见 `reviews/`。这些材料解释“发生过什么”，本文只保留仍影响当前
项目理解的结论。

发生冲突时，源码和配置格式决定实现事实，Git 决定代码基线，当前构建的哈希
决定产物身份，当前实机记录决定硬件结果；本文应随这些权威来源更新，而不能
反过来覆盖它们。自动测试结果和历史构建的真机结果也不能自动继承给新构建。

## 1. 项目背景与当前目标

RC003 原本是面向电视/机顶盒的蓝牙语音遥控器。Windows 可以把它的一部分
按键当作 HID 键盘输入，但不会自然把它的 ATVV 蓝牙语音流暴露成普通麦克风；
部分 HID usage 还会在 Windows 键盘栈中丢失。因此，这个项目不是简单的
“快捷键映射器”，而是一座同时连接 BLE 语音、HID 按键、Windows 输入注入和
音频端点的本地桥。

当前目标用户是希望把小米蓝牙遥控器 2 Pro / RC003 当作 Windows 无线遥控器
和语音入口的人。项目当前优先解决四个结果：

- 13 个实体按钮可以稳定识别、检测并映射为 Windows 动作；
- 实体话筒键不仅能唤起宿主语音界面，还能把实际语音送到宿主选择的输入链；
- “按住说话”是 RC003 唯一正式实体语音生命周期；最后一条 On-request 真机探针
  未收到启动信号后，遥控器点按持续语音和专项程序已经撤下。当前宿主入口都保持
  实体按下对应快捷键按下、实体松开对应快捷键抬起；
- 后台桥接可见、可退出、可重启，并能在失败时留下不泄露设备身份的诊断证据。

基础按键与语音可靠性是当前主线。2026-09-11 本机已通过连击、方向跳格和菜单
双触发修复；新日志重载、断连重连及异机兼容仍待验证。版本值以 `VERSION` 为准，
源码与本地包不因版本值相同就具有相同内容。Quicker 已有可选 URI 动作出口，
但不是核心依赖，也不做双向深度集成。

## 2. 用户工作流

1. 用户先在 Windows 中配对 RC003，并安装或选择需要的音频路由；
2. 在设置窗口中选择 RC003 和语音输出端点，维护一套按住说话快捷键；实体
   话筒键可选择按住说话或普通动作，其他按键只选择普通动作或组合键；
3. 设置按职责自动保存，再启动遥控器服务；默认关闭窗口隐藏到通知区域，服务继续运行；
4. 按钮经 HID/Raw Input 识别后执行主映射；普通动作进入 Windows 动作分发，
   实体话筒键的按住说话动作同时驱动对应快捷键和 BLE/ATVV 音频会话；
5. 用户从通知区域重新打开设置或正常退出桥接；异常断连由监督器清理后重连。

典型语音路由为：RC003 -> BLE/ATVV -> Remote Mic 解码与播放 -> `CABLE Input`
-> VB-CABLE -> `CABLE Output` -> 输入法或语音应用。VB-CABLE 是可选外部驱动，
不是程序自动安装或静默配置的内置麦克风。

## 3. 当前范围与非目标

当前范围：

- Windows 10/11 64 位上的 RC003 配对、连接、13 键识别与动作映射；
- ATVV 语音接收、ADPCM 解码、PortAudio 输出和可选 VB-CABLE 路由；
- 实体话筒键 HOLD 生命周期和各语音程序独立记忆的宿主快捷键；
- 设置、诊断、托盘控制、免安装包和 Windows 自动检查；安装器仅保留历史维护。

当前非目标：

- 不推倒重写已经存在的 BLE、ATVV、Raw Input、Frida、SendInput 和音频链；
- 不把 Quicker 变成核心依赖，也不做双向深度集成；
- 不把保留的 DJI Mic 2 底层兼容代码扩展成另一条硬件开发主线；
- 不自动改变系统默认音频设备，不静默提权、安装驱动或绕过安全软件；
- 不把未签名候选、自动测试或旧候选的结果描述成全面发布验收通过。

## 4. 来源、演进基线与许可

- Remote Mic 主项目调研快照：`HD838A/remote-mic-app` 的 `main` 在
  2026-08-20 核验为 `4d526175817ad2c4c5fbe1650d528c946a3cdbf3`，主要维护
  macOS，不是本 Windows 客户端的直接代码基线。
- 当前 Windows 公共仓库：`ZSTDJan/windows-remote-mic-app`；本地维护历史最初从
  `miaomiaozii/windows-remote-mic-app` 派生。
- Windows 派生起点：`271ed7947eec19c4c691ed3ba97f338461be8051`；内部实现
  基于 GPL-3.0-only 项目 `nijez/open-voice-bridge` 的 Windows RC003 客户端。
- 初期完整源码检查点：`eafd203`；该提交只证明对应批次的代码与自动检查基线，不代表
  最新冻结包已经完成真机验收。
- 当前维护主线：小米蓝牙遥控器 2 Pro / RC003 的 Windows 客户端。
- 协议与 HID tap 参考：GPL-3.0-only 项目
  `xxb26553663-star/remote-bridge-hub` 的提交
  `8a93f321ac71a602300c6cd77f7256fa4b63068e`；具体归属见
  `THIRD_PARTY_NOTICES.md`。
- 本仓库许可：GPL-3.0-only。PySide6、Frida、PortAudio、NumPy、WinRT 投影、
  VB-CABLE 等仍按各自许可和通知分开处理。
- Quicker 不是核心运行依赖。现有 URI 动作出口不应反向侵入
  BLE、HID 或语音状态机。
- DJI Mic 2 不再提供设置或自动诊断入口；设备档案和端点识别只保留作旧配置兼容，
  不复用 RC003 桥接链路。

## 5. 统一术语

| 术语 | 本文含义 |
| --- | --- |
| RC003 | 小米蓝牙遥控器 2 Pro；本项目当前唯一桥接主设备 |
| 桌面主进程 | 当前登录会话唯一的 `RemoteMicRC003.exe`；持有 Qt 窗口、通知区域、桥接 worker 和元素导航 |
| 桥接 worker | 桌面主进程内持有 BLE、按键和音频资源的工作线程；不是第二个长期进程 |
| HID/按键链 | 从 Windows HID、Raw Input 或 HID tap 到逻辑按钮和映射动作的链路 |
| ATVV/语音链 | 从 BLE GATT 控制与音频特征到 PCM 输出端点的链路 |
| 宿主 | 最终接收快捷键和虚拟麦克风输入的输入法或语音应用 |
| HOLD | 实体话筒键按下时发送宿主 key-down，物理松手时发送 key-up；`AUDIO_STOP` 是缺少松手边沿时的释放兜底 |
| 历史 TOGGLE | 已撤下的开关型实验路径；仅保留配置迁移和 `BUG-017` 调查证据，不再是可选能力 |
| 候选构建 | 未签名、需要绑定源码提交和哈希、尚可能等待真机验收的包 |

内部 Python 包名 `ovb_rc003` 为现有导入、启动与构建入口保持兼容，不是另一个产品。

## 6. 可执行程序角色

冻结目录包含一个用户入口 `RemoteMicRC003.exe`，参数决定桌面角色；另有一个
只供安装器、权限修复和固定计划任务调用的 `RemoteMicRC003HidHelper.exe`：

| 入口 | 角色 | 是否持有硬件资源 |
| --- | --- | --- |
| 无参数、`--settings` | Qt Quick 桌面主程序 | 可长期持有窗口、通知区域、桥接 worker 和元素导航 |
| `--background` | 隐藏启动同一个桌面主程序 | 与无参数入口相同，只是不立即显示窗口 |
| `--bridge` | 兼容启动形式 | 隐藏进入同一个桌面主程序，并请求启动进程内桥接 worker |
| `--request-exit` | 隐藏的安装维护入口 | 只写入一次性完整退出请求并等待桌面主进程结束 |
| `--dry-run` | 模块导入检查 | 否 |
| `--help` | 帮助文本 | 否 |
| `--diagnose-ble-candidates <result>` | 隐藏的有界 BLE 诊断子进程 | 短暂持有 WinRT BLE 枚举资源 |
| `--list-remote-devices <result>` | 隐藏的已配对设备列表子进程 | 只读枚举并输出受限摘要，不配对或打开 GATT |
| `--rc003-hid-injector --pid ...` | 隐藏的受限注入子进程 | 短暂持有目标进程句柄 |
| `--element-navigation` | 旧独立导航兼容入口 | 仅为旧启动形式保留；正常产品导航已嵌入桌面主进程 |
| HID 助手 `--install-task` / `--uninstall-task` | 经 UAC 明确启动的安装或卸载动作 | 写入/删除 Program Files 助手和固定计划任务 |
| HID 助手 `--inject` | 计划任务按需启动的管理员注入动作 | 短暂持有已独立定位并核验的 RC003 WUDFHost 句柄 |

HID 助手不接受调用方传入的 PID、程序路径或配置路径。它只从当前账号的正式配置入口
读取并严格校验设备选择摘要，不调用配置归一化或执行其它用户设置；匹配到所选 RC003 后
允许进入共享宿主，但报告接管必须通过下述来源合同。旧非热更新组件的设备迁移仍核验
宿主独占性。计划任务没有真实触发器，只能运行
Program Files 中的固定助手和固定 `--inject` 参数；Windows 回读时允许补出一个直属、
无属性、无内容的空 `Triggers` 容器，任何子触发器、自动维护或失败重启设置都视为合同
损坏。目标架构是普通权限主程序、桥接 worker 与登录自启；公开版仍按首页临时要求管理员启动。
当前任务归属正在登录的管理员
账号，不支持标准账号借用另一管理员账号凭据后继续无提示调用。

源码运行使用当前 RC003 目录的 `.\.venv\Scripts\python.exe -m ovb_rc003`。PyInstaller 不直接分析包内
`__main__.py`，而从顶层 `src/launcher.py` 做绝对导入，避免冻结入口失去
包上下文。

桌面主程序由 per-session Windows named mutex 保证单实例；重复双击、开始菜单、
任务栏或通知区域入口只恢复已有窗口，不会再创建一套窗口、托盘或桥接。桥接 worker
另持有旧桥接 mutex，防止升级期间尚未退出的历史独立桥接同时抢占 BLE、Raw Input、
HID、合成按键或音频资源。关闭窗口默认先释放设置页临时按键录入/检测，再隐藏到通知
区域；通知区域和桥接 worker 继续由同一进程持有。用户明确选择“完全退出”时，才按
正常清理顺序结束全部长期资源。

产品级 mutex 继续跨版本、跨目录互斥；每个“版本加可执行文件位置”另持有哈希运行
身份 mutex，用于区分“同一副本重复双击”和“用户明确点击了另一副本”。可见启动命中
其它身份时，先取得全局交接 mutex，再询问用户并等待旧版正常退出；同一登录会话只允许
一个交接等待者。退出请求携带本次交接的唯一编号，只有确认旧版已退出且该请求已被消费
或安全清除后，新副本才重新取得产品级 mutex 并启动。更早的不支持请求的旧版只会被
唤出，等待用户从通知区域选择“完全退出”，不会被强杀。

桌面主程序取得单实例所有权后、创建窗口前，还会迁移可能残留的历史独立桥接。只有旧服务
已经正常停止且旧运行状态已安全清理，当前版本才继续打开并在主进程内恢复桥接；无法确认
停止、清理失败或旧服务在交接时重新出现，都会阻止新版打开并要求用户完全退出旧版后重试。

安装器、卸载器和 Start Menu“停止”入口使用 `application-exit-request.json` 请求桌面主
进程走同一套完整退出路径。当前版本会先处理未保存提示，再停止输入监听、诊断任务、
桥接 worker、BLE、HID、语音热键和音频资源；请求处理期间禁止新的桥接启动或自动恢复。
请求被取消、拒绝或超时后，安装器不会覆盖正在使用的程序文件。

桥接 worker 是运行状态的唯一写入者。Windows 文件名包含当前登录会话 ID，与
`Local\\` 桥接 mutex 的隔离范围一致；会话查询失败时固定退回当前进程范围，不会在运行中
改换路径。schema 2 每 5 秒原子更新版本与构建摘要、BLE 连接、Raw Input、HID tap、
语音按键通道、最近按键和语音活动；不写设备地址、窗口标题、输入文字或语音内容。设置
控制器只读：能确认其它构建，或 Raw Input 与 HID tap 同时明确失败且语音空闲时，最多
正常恢复一次；语音按键通道失败只提示用户重新启动，不自动停止服务。旧 schema、心跳滞后
或语音进行中也不自动停止。停止旧服务后，状态清理先原子认领文件再核对旧 PID，避免退出
中的旧进程删除新进程刚写入的状态。

`--bridge-from-settings` 只是旧启动命令保留的隐藏兼容标记，不是独立运行角色。
`--bridge` 命中已有主程序时只写入一次短时启动请求，由现有进程消费。BLE 诊断另起子进程，
是为了在 WinRT 调用卡住时仍能由父进程确认终止，不把不可取消的原生调用留在
桌面主进程中。

## 7. 运行拓扑

```text
RC003 遥控器
  |-- BLE GATT / ATVV --------------------------+
  |                                             |
  |-- Windows HID 键盘栈 --> Raw Input --------+--> RC003App
  |                                             |      |
  +-- HidOverGatt read --> WUDFHost + Gadget ---+      |-- 按键动作 --> SendInput / 应用启动
                                                        |
                                                        +-- ADPCM --> PCM --> PortAudio
                                                                            |
                                                                            +--> CABLE Input
                                                                                  |
                                                                                  +--> 宿主语音输入

桌面主进程 -- config.json / key_bindings.json -- 进程内桥接 worker 热加载
设置控制器 -- 一次性 key-detection 文件 IPC --- 进程内桥接 worker 捕获下一键
桥接 worker -- 原子运行状态 + 5 秒心跳 -------- 设置页版本/通道校验
桥接 worker -- app.log ------------------------- 设置页/用户诊断
桥接 worker -- schtasks /Run 固定任务 ---------- 管理员 HID 助手 --> 核验并注入 WUDFHost
桌面主进程 -- Qt 事件队列 ---------------------- 内嵌元素导航运行对象
```

这里有两条相互独立、在主映射解析为语音动作时汇合的数据链：

- HID/按键链负责判断按下了哪个实体按钮，并执行 Windows 动作；
- BLE/ATVV 链负责打开遥控器麦克风、接收压缩音频、解码并播放到选定端点。

实体麦克风键可能同时从 ATVV、Windows F5、Raw Input 和 HID tap 到达。因此
`app.py` 有跨来源手势门闩，必须把同一次实体按下合并成一次动作；该动作可以
是语音，也可以是用户为麦克风键配置的普通映射。来源边沿可能重叠，也可能在
首个来源已经释放后迟到；两种情况都必须在有界保护期内折叠，不能把迟到来源
误判为下一次点击。

语音会话由已识别的遥控器输入和 ATVV 音频状态仲裁；不能把普通实体键盘的 F5
当成遥控器语音授权。来源必须匹配对应的按下和抬起，迟到或游离边沿不能被当作
下一次操作。旧 F5 专用钩子的历史方案不再是当前接管入口，见 8.2 节。

## 8. 按键与动作映射链路

### 8.1 输入来源

`raw_input_windows.py` 注册并读取 Windows Raw Input，解析标准键盘事件并
匹配 RC003 设备。Windows 键盘栈会丢掉返回、音量加、音量减等部分 usage，
所以 `frida_compat.py` 提供可选 HID-over-GATT tap 补齐缺失报告。

HID tap 的 loopback TCP 客户端是运行在已核验目标 `WUDFHost.exe` 内的
Frida Gadget。服务端通过 `GetExtendedTcpTable` 核对 TCP 客户端进程 PID；
无法确认或 PID 不等于刚核验过的 WUDFHost 时，连接会在读取任何消息前关闭。
日志记录固定原因码、关联编号、PID、计数及必要的系统/组件指纹；不写设备路径、
蓝牙地址或输入文本。普通失败日志与可选逐报告诊断的范围见 `MAINTENANCE.md` 的
“HID 拦截失败日志补全”。

冻结安装版由普通权限桥接 worker 调用预先登记的固定计划任务；管理员助手自行重新定位
RC003 的 WUDFHost、启用调试权限、复核进程名和固定 Gadget 哈希后注入。源码调试
或手动以管理员权限运行的便携版仍可走原有直接注入子进程。助手、任务、哈希或
Gadget 连接任一项不符合预期时，tap 明确失败，不回退为未经核验的注入。

### 8.2 去重与原生按键归属

同一个按键可能同时被 HID tap 和 Windows 原生键盘路径报告。如果两条路径都
继续传播，用户会同时得到原生字符/功能和映射动作。当前正式归属点不再是看不到
设备身份的通用低层键盘钩子：Frida Gadget 在已经核验的 RC003 `WUDFHost.exe` 内，
于 `NtDeviceIoControlFile` 的 UMDF 输出复制入口保存原报告并清空 usage。当前确认
的条件是 IOCTL `0x80018483`、8 字节元数据、operation=2、selector=1 和 9 字节报告。
清空必须发生在 `onEnter`，因为内核在调用返回前已复制源缓冲区；旧 `onLeave` 清空
太晚，不能阻止原键。成功返回后才把保存的报告交给 Python，沿用手势去重与映射；
复制失败恢复源缓冲区且不派发该报告。

协议 4 增加来源能力版本 1，固定助手代际 12。生产客户端要求新能力并发送所选摘要，
不能把协议号相同的旧脚本当成支持设备选择。来源观察入口读取 WUDFHost 的
`DeviceIoControl` 实际导入槽；不能固定在 KernelBase，也不能把 Frida 推导的导入
address 当作槽内地址。目标须对应 Kernel32/KernelBase 的同名导出或 Windows 实际
解析的同名入口；槽缺失、越界、多义或目标不可核验时不猜入口，原来源拒收与独占
兼容边界继续生效。观察发生在系统包装函数改写调用者寄存器之前，仍须完整核验
宿主调用点、设备对象、控制句柄和所选身份。`DeviceIoControl` 的调用点通过 Windows
函数展开表与实际指令确定设备对象寄存器及控制句柄字段；只读验证 WUDF 设备栈接口的
QueryInterface IID 比较和第 31 槽纯字段 getter。该 getter 可以返回 Device Parameters，
因此先用 NtQueryKey 核验 HKLM/SYSTEM/控制集/Enum 的完整实例路径：实例根直接读取，
参数子项仅以 KEY_QUERY_VALUE 打开其自身实例根读取 ContainerID，并关闭自行打开的
句柄。其它路径和更深子项拒绝，不能采用参数子项里的同名值或搜索其它设备身份。
不依赖 C++ RTTI，不调用未知 C++ 方法，不按键值、时间、名称或枚举顺序猜测来源。
摘要与 `remote_selection.container_key` 一致，原始 ContainerID 不持久化或记录。
助手准备脚本与宿主热更新异步完成；首次注入截止时间内的旧握手保持禁用并等待替换
连接，不重新注入或延长截止时间；超时仍按旧能力不匹配处理，真实旧组件迁移限制不变。

外层调用与内层复制按线程栈、句柄、输入/输出指针和长度关联。非所选来源在报告读取、
原文诊断、候选占用和清空前过滤；选中来源首次候选不修改，客户端复核宿主后绑定。
无法解析设备对象时，仅保留经重新核实的独占宿主兼容路径；候选期间出现第二只设备则
撤销该兼容候选，保留同一连接等待设备来源证明。共享宿主不能使用独占回退。
旧按住已进入 Windows 时等待其完整松开，随后接管下一次操作。句柄关闭、选择切换、
租约失效或失联使旧绑定失效；已绑定来源无法核实时取消旧手势，不完成未知松键动作，
不因此重启服务。来源失败日志仅保存固定阶段码及 Windows 错误码。原来的复制失败恢复、
双输入抑制和不干扰其它键盘的边界不变。

此方案沿用既有管理员助手与 Frida，不安装新过滤驱动，不修改 Windows 蓝牙配置或
宿主池化设置；它依赖已经验证的 Windows 内部复制语义，不是微软承诺稳定的公开
接口。蓝牙版本号本身不能判定兼容，异机须核对 Windows/驱动链和真实报告；不支持的
指令或接口布局安全拒收。共享来源隔离验证见 `CHECK-HID-SOURCE-001`，尚不代表异机
实测通过。2026-09-11 本机已确认重复输入修复有效，2026-09-12
按用户要求记录收口；具体证据与未验证边界以 `CHECK-HID-COPY-001` 为准。新日志
组件尚需单独加载验证，不能用预期脚本版本冒充已加载版本。

全部按键采用同一降级规则：只有 HID tap 已确认接管原始报告时，才执行自定义单击、
双击和长按；旧遥控器组合配置仅保留兼容、不执行。tap 未能接管时，全部自定义按键映射停用，只保留 Windows
原始按键操作，避免原始操作和自定义操作同时执行。Raw Input 此时只负责跟踪已经到达
Windows 的物理边沿，不执行映射。tap 从 `READY` 变为失联或异常时，应用在同一个输入
仲裁锁内先取消活动手势并补齐所有已认领按键的松键，再撤销 tap 所有权；恢复时等待
完整松开后再重新接管，防止最后一次按下没有对应抬起而造成连发、半个组合键或状态卡住。

旧实现曾用全局 F5 钩子处理原生话筒键；不能再把“桥接运行期间所有实体 F5 都被吞掉”
描述成当前合同。当前 `VoiceKeyPhysicalizer._hookproc()` 维护物理状态和有标记的语音
注入边沿，并保留旧方向所有权处理；协议 4 不登记该旧方向窗口。原生遥控器报告的
抑制以本节提前拦截为准。Raw Input 中 F5 到 `mic` 的设备映射仍保留，
不能据此认为钩子能识别任意 F5 的设备来源，也不能把 F5 自动转换成宿主快捷键。

### 8.3 物理签名、手势与动作

`raw_input_windows.physical_signature()` 把可移植的键盘/HID字段组成签名；
签名不包含设备接口路径。`physical_bindings` 可把未知签名学习为逻辑按钮。

`button_gesture.py` 在单击、双击、长按和可重复按住之间做纯状态机判定。
连接清理会递增 generation 并取消定时器，旧连接的延迟回调不能在重连后产生
动作。

`key_mapping.py` 保存语义动作，而不是界面文字。`app.py` 再把语义动作交给：

- `win32_input.py`：批量 `SendInput` 普通键、组合键、媒体键和鼠标单击/滚轮；
- `action_executor.py`：不经过 shell，以参数数组启动已解析应用；
- 专用语音状态机：只接受实体话筒键的 `VOICE_HOLD` 主动作，即时处理按下与
  释放边沿，不等待双击或长按判定。

麦克风键配置为普通动作时仍可使用单击、双击和长按。只有方向、退格、音量和滚轮
语义动作允许按住重复，且滚轮仍受实体键连发白名单限制；自定义组合键无论放在方向键还是其他键上，都只执行
一次完整点按，避免定时重放修饰键组合。

配置迁移先执行产品边界归一化。旧开关型、非话筒键语音或次级手势语音会把
对应整键标记为停用，连同该键的单击、双击和长按一起停止执行，直到用户在
界面中重新选择并由界面自动保存；不会悄悄转换成按住说话。

鼠标动作使用物理左、右、中、X1、X2 的 Win32 标志，不跟随主次按钮交换；滚轮一次
发送一个 `WHEEL_DELTA`。点击只操作当前指针，不接管本机鼠标 Hook，也不改变元素
导航的定位和点击算法。

按键映射热加载由独立映射锁串行化。单击、双击、长按、连发、普通话筒动作和遥控器
组合从物理按下开始持有当前映射；已经产生但尚未执行的计时器回调也算活动手势。新配置
只有在语音状态和全部普通手势都空闲后才替换运行中映射。

如果 `SendInput` 部分成功且补偿 key-up 或 mouse-up 也失败，应用会保留“仍欠释放”的
具体键盘组合或鼠标键。下一次动作、BLE 清理和最终退出都会先尝试安全释放；完成前
不会继续注入新动作。滚轮没有持续按下状态，不进入欠松开账目。

元素导航的鼠标输入有独立所有权：需要移动指针前先检查五种实体鼠标键，任一按住时
都不移动；点击按下与抬起同批提交并确认数量。补偿抬起仍未确认时保留具体按钮，后续
点击、滚轮、离开导航和进程停止都先重试，完成前不继续鼠标操作。

2026-09-14 自身窗口导航增加本地接收。`_LocalNavigationInput` 复用原导航动作表
和队列：只有导航接管中、目标 PID 为自身且实际前台等于扫描根窗口时才接收。
遥控器语义映射被接受后不再注入系统；自定义组合键及外部窗口维持原发送/钩子路径。
设置 controller 的 Qt 键盘分支位于录入/检测之后，作为窗口消息输入的补充入口；
已被全局钩子吞下的输入不会进入该分支。Qt 按下/松开所有权与 RC003 原始边沿关联
分离，退出后的已认领松键仍完成清理，不将普通字符或修饰组合认领为导航。

## 9. 语音链路

### 9.1 BLE 连接与 ATVV 协议

`ble_transport_winrt.py` 使用 WinRT 枚举已配对 BLE 设备，并通过 UNCACHED
GATT 查询连接 ATVV voice service。生产入口先按用户选择的 ContainerId 摘要过滤，
只连接唯一匹配的 RC003；所选设备消失或身份有歧义时失败关闭，不探测或改选其它设备。
摘要与持久化合同由 `remote_selection.py` 持有；Raw Input 接口经 PnP 属性取得
ContainerId，HID 宿主定位经节点 ContainerID 取得同一身份。共享宿主内部报告来源无法
核实时仍拒绝接管，并显示 `selected_device_shared_host`；这不等于必须重启电脑。
独立诊断的未传选择参数路径保留旧多候选 ATVV 探测，生产桥接不得使用该路径。

连接后主要特征为：

- TX：主机发送 capabilities、`MIC_OPEN`、`MIC_CLOSE`；
- CONTROL：设备发送 capabilities、麦克风按钮、音频开始/停止；
- AUDIO：设备发送 ADPCM 帧。

控制与音频分别进入有界队列。控制队列溢出是协议状态不可证明的硬错误，会
停止 worker 并请求重连；音频队列满时只丢最旧音频并计数，不能让音频洪峰
挤掉关麦等控制事件。两个队列的通知共享单调序号，序号分配与入队由同一个
producer lock 保护。一般控制仍优先，但 `AUDIO_STOP` 必须先排空同一连接代、
序号更早的音频；否则 decoder 会先关闭并把已经收到的尾包误判为晚到数据。

### 9.2 解码、统计与输出

`atvv_protocol.py` 实现 capabilities、命令与 IMA/DVI ADPCM 解码；
`atvv_session.py` 管理 session id、decoder reset、sync、晚到音频保护和
隐私安全 PCM 统计。统计只包含帧数、样本数、峰值、能量、过零等数值，不
保存或转写语音内容。

`audio_playback.py` 使用 `sounddevice`/PortAudio 打开用户明确选择的输出
端点：

- WDM-KS 被拒绝；
- 同名端点优先 WASAPI，其次 DirectSound；
- 先真实 open/start/stop/close 预检，再允许设置保存；
- 16 kHz 单声道 PCM 按端点能力转换采样率和声道；
- 流关闭失败时保留对象引用，后续清理继续重试。

`audio_playback_worker.py` 在 BLE 解码回调与 PortAudio 阻塞写入之间维护 64 帧
有界 FIFO。PCM 回调只做不可变副本和快速入队；音频停止使用 FIFO 屏障，
屏障前的块已写完后才执行宿主收尾。队列满、写失败或屏障超时会停止
本轮转发并请求重连。清理必须先停播放 worker，worker 未停止时不得关闭
或丢失它仍可能使用的 sink。

通常输出端点选择 `CABLE Input`，VB-CABLE 再把它暴露为录音端点供输入法或
语音应用使用。软件不会静默安装驱动；设置页只能在用户明确确认后启动随包、
哈希校验通过的厂商安装程序。

### 9.3 宿主语音动作

`voice_controller.py` 只决定实体按住状态，不直接操作 Windows：话筒键按下产生
逻辑 key-down，物理松手产生逻辑 key-up；如果 Windows 没有提供松手边沿，设备的
`AUDIO_STOP` 负责同一个结束边沿兜底。搜狗、微信、豆包和自定义按住型程序都接收
同一实体会话的按下和抬起；运行时保存本次实际发送的快捷键与后端，设置变化不能把
旧会话的抬起发到新组合键。

遥控器点按持续语音实验的最后一条 On-request 真机诊断已经完成，协商成功后仍未
收到实体 `START_SEARCH`。因此该实体运行路径、设置项和专项程序均已撤下，不再由
任何宿主适配层重新提供开关型语音。

`app.py` 强制顺序为：

1. 验证 BLE session 存在；
2. 释放任何历史遗留热键；
3. 开始语音时先验证并打开播放端点；
4. 先向宿主交付热键；
5. 只有宿主 key-down 成功后才允许向设备发送 `MIC_OPEN`。

语音快捷键释放失败时只保留一个退避重试；新会话开始前先处理旧欠松。断连、停止
服务、正常完全退出和 120 秒保险都会再次释放桥接确实持有的键。任务管理器强杀时
进程内代码没有执行机会，仍按已确认决定作为极低概率已知风险保留，不增加外部看门狗。

### 9.4 豆包兼容层

`doubao_rpc.py` 的可选 `DoubaoPhysicalizer` 只对固定名称、固定安装目录且
SHA-256 匹配的 `ImeService.exe` 安装 Frida callback hook。当前核实版本的
SHA-256 为 `94B17BDCA571AC3CD2DAFA687B4CB8E3A789CDA9D044276483003B0A9E8F77A6`，
键盘回调 RVA 为 `0x7426C0`；未知版本直接停止本轮，不猜地址。hook 只处理本桥接
为豆包当前快捷键发送且带专用标记的 injected 边沿，不改变其它按键。右侧 Alt 仍是
新配置默认，但运行时使用自动读取或用户手动录入后保存的实际组合键。自动键序测试
不能代替 UU 远程与目标输入法真机验收。

兼容层只处理 `app.py` 的语音状态机已经取得所有权后发出的宿主动作。legacy F5
本身不会被转换成右侧 Alt 或其它语音快捷键，因此兼容层与 F5 抑制路径之间没有
第二个宿主按键发送者。

Frida script/session 的引用只有在卸载或 session detach 成功后才清除。
session detach 成功即证明其脚本不再被会话持有；单独 script unload 报错不再
把兼容层永久误判为“仍需清理”。

### 9.5 搜狗本机内部 IPC 调查（未实施）

2026-08-24 只读核验当前机器安装的搜狗输入法 `16.6.0.4777` 及语音组件
`1.0.1.3272`。证据来自
`Components\ai_voice_input\1.0.1.3272\bin\resources\app.asar` 内的
`out/main/chunks/NativeIPCService.win32-D7_cisol.js`、`out/main/index.js`，以及
当日搜狗运行日志。当前版本会创建标题为 `SogouVoiceInputWnd` 的 0 尺寸隐藏
窗口；该字符串是窗口标题，不是固定窗口类名。窗口监听 `WM_COPYDATA`，其中
`dwData=0` 表示命令行消息，正文按 UTF-8 解码后解析参数。

发送 `--from=hotkey` 会进入搜狗自己的 toggle 路径：空闲时切换为开始，录音时
切换为结束。当前代码中没有找到供外部调用的独立“开始”“停止”或录音状态查询
命令，也没有能证明录音已经真正开始或结束的结果回执。因此按下和松开各发送
一次只能在两次消息均未丢失、未重复且搜狗没有自行改变状态时模拟按住说话；任一
条件不成立都可能反相，例如松手时反而开始录音。

`WM_COPYDATA` 是发给本机窗口句柄的进程间消息，不能从控制电脑直接穿过 UU
到达另一台被控电脑。只有 Remote Mic 与搜狗位于同一台 Windows 电脑和交互
会话时，这条路径才可能直接使用；跨电脑需要被控端另有接收程序。它也只绕过
快捷键触发，不解决音频端点无声、输入框兼容或文字上屏问题。

当前决定是保留调查事实、暂不进入产品实现。本次没有向搜狗发送测试消息，尚未
验证真实时延、漏发/重复、搜狗卡死或重启、不同权限、升级兼容和 UU 两端部署。

### 9.6 搜狗进程确认与语音结果诊断

当前实现不采用 9.5 的私有窗口消息，也不再调用 `--auto-launch` 预热。搜狗和自定义
程序按用户保存的设置启动后，只在短截止时间内确认目标进程。每次搜狗语音按下前最多
等待 0.6 秒确认真实进程；进程不存在就停止本轮，不发送快捷键或 `MIC_OPEN`。程序不
检查可见窗口，也不会为了就绪判断重复启动或重复发送用户快捷键。

统一结果诊断在宿主快捷键前后读取前台 PID、窗口/焦点控件类名和
`WM_GETTEXTLENGTH`，单次文本长度查询最多等待 20 ms。它不读取窗口标题或文本内容，
也不会因为焦点或长度没有变化而自动重发、粘贴或改变输入法设置。
后续若重新评估，应先做隔离探针并使用有超时的发送方式，再分别验证正常切换、
漏一次、重复一次、中途自停和版本变化；在取得用户明确确认前不得替换现有宿主
快捷键逻辑。

## 10. 设置、配置与热加载

运行期目录固定为 `%LOCALAPPDATA%\RemoteMic\RC003`：

| 路径 | 内容 |
| --- | --- |
| `config.json` | 设备选择、增益、重试、按住说话快捷键、输出端点 |
| `key_bindings.json` | 主/次手势动作、可移植物理签名映射 |
| `logs\app.log` | 轮转运行日志 |
| `logs\hid-helper.log` | 普通主进程一次写入并立即关闭的管理员按键组件结果 |
| `bridge-runtime-status-s<会话 ID>.json` | 当前 Windows 登录会话的桥接心跳、连接与输入通道状态 |
| `key-detection\` | 最多 30 秒有效的一次性按键检测 IPC |
| `application-exit-request.json` | 安装、卸载或明确停止操作使用的一次性完整退出请求 |
| `updates\<版本号>\` | 已校验的安装器或便携 ZIP；`.part` 只在下载未完成时临时存在 |
| `captures\` | 用户显式运行诊断工具时生成的隐私安全 JSONL |

两份 JSON 都逐文件使用临时文件、flush、`fsync`、`os.replace` 原子替换。
设置页保存两份文件时走 `config.save_settings_pair()`：先验证两份结构和隐私
字段，第二份保存失败就把第一份按原始字节原子恢复。这样普通的文件占用、
权限、序列化或 replace 失败不会留下混合版本。

“按键”页以编辑弹窗的“完成”为提交边界：一次弹窗内的主动作、双击、长按和备注变化
合并为一次自动保存；恢复内置默认确认后也走同一保存链。桥接启动、端点预检、驱动操作
或语音快捷键写入占用期间只延后落盘，不丢弃界面修改。失败时保留脏状态并开放显式重试；
保存期间又发生修改时废止旧候选，再保存最新 revision。

边界说明：普通 NTFS 文件 API 不提供两文件共同提交的断电原子性；极端断电
仍不等同于数据库事务。当前设计解决可观察的运行期写入失败，并通过回读确认
最终内容；未来若需要断电事务，应引入显式 journal/schema revision，而不是
继续堆叠临时文件技巧。

桥接按 mtime 一起热加载按键动作和按住说话设置。活跃语音会话不会中途切换
状态机，新设置会等本次手势完整收尾后应用。设备 profile 与音频端点仍要求
重启桥接。若新文件无法解析，运行时保留最后一份有效配置。F5 防漏不依赖语音
快捷键配置，低层钩子不会把 F5 转换成任何宿主快捷键。

配置版本以 `config.SCHEMA_VERSION` 为准。当前语音设置只写按住说话快捷键、分程序记忆及其默认/自动/手动
来源；旧开关型语音不会被解释成按住说话。桌面行为分别保存自动启动桥接和关闭窗口方式：新配置、缺失值
或非法值默认“隐藏到通知区域”，已有合法的“隐藏到通知区域”或“完全退出”选择原样
保留。运行时迁移标记不会写回用户文件，只有用户明确重新选择并由界面自动保存后才生成新配置。

`remote_selection` 使用独立子版本 1，只保存已添加设备的 ContainerId SHA-256 摘要、
支持的 profile 和已登记的 active key；不保存原始身份。旧配置缺失该项时不自动选择。
设备切换不热改现有 worker 的身份：先确认旧服务退出，再原子保存并按原运行状态启动。
保存回退失败阻止启动；移除当前只清软件记录并停服，不删除 Windows 配对。

配置隐私守卫递归拒绝地址、设备 ID、设备路径、接口 ID 和 token 等字段，
且大小写不敏感。诊断捕获也不提供“包含设备路径”开关。

## 11. 设置界面与诊断

`qt_settings_app.py` 把 Python model/controller 暴露给 QML。主要页面：

- `DevicePage.qml`：设备、按键接收、遥控器服务、运行日志、更新和桌面行为；
- `ButtonsPage.qml`：13 键整宽矩阵，按单击/双击/长按统一展示和编辑主/次动作，
  通过共享 selected state 承接真实按键检测定位，编辑完成后自动保存映射；
- `VoicePage.qml`：虚拟音频、输出端点、语音程序、分程序快捷键和实际说话验证；
- `main.qml` / `Tokens.qml`：窗口、三页导航和设计 token。

设置窗口采用一套固定全局框架：顶部保留产品标识与三页导航，中间页面独立滚动，
底部只显示一份 `SettingsController` 普通状态或错误。普通状态使用中性色，不把
“进程已启动”着色或措辞成“RC003 已连接”；错误优先于普通状态，后续成功操作
会清除旧错误。各设置按职责自动保存；设备页“启动服务”只使用最后已保存的设置，
不顺带提交其它页面尚未完成的修改。

不再设置独立权限页或诊断页。蓝牙、按键接收、HID 助手、VB-CABLE、输出端点和
语音程序状态回到对应业务行；需要人工判断的项目明确标为人工验证，不合成统一的真假
授权状态。自动重新检查不会主动播放测试声音，声音通道测试只由用户点击启动。

耗时 WinRT、PortAudio 和系统诊断在后台线程执行。窗口退出会按顺序停止热键
录制、真实按键检测、诊断 worker 和遗留音频预检流；某一步失败不跳过后续
清理。

### 11.1 手动更新边界

软件更新的正式来源是 `application_update.py`。只有用户点击“检查更新”后，设置
控制器才在后台 worker 访问固定的 `ZSTDJan/windows-remote-mic-app` GitHub Releases；
启动、后台运行和定时器均不触发联网，客户端不保存 GitHub 凭据。Release tag 只作
发布编号，版本比较严格读取同一次 Release 中安装器与便携 ZIP 文件名携带的内部版本；
资产、`SHA256SUMS.txt`、预发行状态或版本不能互相印证时，该 Release 不可用。
新发布仅包含免安装 ZIP 和校验清单，不再要求安装器；新 tag 与 VERSION 一致，
命名规则见 `VALIDATION-AND-DELIVERY.md`，历史 tag 与资产格式继续兼容。

API 请求只允许 `api.github.com`；资产初始地址必须属于本仓库的 `github.com` Release，
重定向只允许 GitHub 资产域名。响应大小、资产大小均有上限，单次网络等待有超时。
便携版或源码运行选择 ZIP；旧安装版在 Release 没有安装器时也选择 ZIP，
仅历史 Release 有安装器时保留旧选择。下载写入
`%LOCALAPPDATA%\RemoteMic\RC003\updates\<版本号>\<文件名>.part`，依次核对 API
大小、`SHA256SUMS.txt`，以及 GitHub 提供时的 SHA-256 摘要，全部通过后才原子改为正式
文件名。取消、中断、校验失败和写入失败会清理临时文件。

当前公开版本可能按临时说明以管理员身份运行，因此更新链路绝不执行下载文件，也不
自动关闭、覆盖或重启程序。界面只提供“打开文件夹”，安装器和便携 ZIP 都由用户在
完全退出当前版本后手动处理。

后台桥接存在时，“检测真实按键”不再争抢 Raw Input/HID tap，而是写入一次性
本地文件请求，由桥接吞掉下一次按下/释放、返回逻辑 button id 且不执行映射。
无法安全确认桥接 mutex 状态时，设置页停止检测，不猜测“没有运行”。

13 个实体按键的主映射统一可编辑；只有实体话筒键额外提供“按住说话”，其他
按键只提供普通动作或组合键。语音页按程序分别记忆一套按住型快捷键，并记录默认、
自动读取或手动录入来源；普通自动读取不覆盖合法手动值，用户主动刷新成功时才切回
自动来源。保存层拒绝把语音动作放入非话筒键或次级手势；麦克风键默认映射为
按住说话，但可以改为普通动作。

多个输入来源可能同时尝试认领该请求。`key_detection_bridge.py` 必须先用
`O_CREAT | O_EXCL` 独占 claim lock，再读取和移动 request JSON；结果写入失败
时尽量恢复 request。先读文件再争锁会在 Windows 共享时序下产生双失败。

## 12. 线程、队列与资源所有权

| 所有者 | 资源 | 停止/失败原则 |
| --- | --- | --- |
| asyncio 主线程 | supervisor、BLE 生命周期、托盘退出协调 | cleanup 失败则停止重连循环 |
| BLE worker | CONTROL/AUDIO 队列和解码回调 | 异常通知 supervisor，不静默死亡 |
| 音频播放 worker | 64 帧 PCM FIFO、停止屏障和阻塞式 PortAudio 写入 | 队列满/写失败/屏障超时停止转发并请求重连；未停止时保留 sink |
| Raw Input 线程 | 隐藏窗口、设备通知、按键状态 | join 超时保留 listener 引用 |
| 低层钩子线程 | 物理键状态、标记语音边沿与旧方向所有权兼容 | 消息队列 ready 后才报告启动成功；协议 4 原键拦截在 HID 宿主 |
| HID tap 线程/管理员助手 | loopback server、固定计划任务、目标进程句柄、Gadget 消息 | 助手无动态目标参数；验证任务 XML、助手哈希和客户端 PID，心跳/大小有界 |
| 托盘线程 | Win32 window、图标、菜单 | 退出回投 asyncio，不直接碰 BLE |
| Qt 诊断线程 | 一次诊断任务 | 窗口退出发 stop 并有界等待 |
| Qt 更新 worker | 用户手动触发的一次检查或下载 | 进程销毁时发 cancel；可逆退出失败前仍结算结果，真正销毁后丢弃迟到结果；不执行下载文件 |
| PortAudio sink | native output stream | close 未确认时保留 owner 重试 |

最重要的总原则是“引用代表所有权”。线程仍活、handle 未关闭、stream 未关闭
或 key-up 未确认时，对象引用不能清成 `None`。清理会尝试所有独立步骤，最后
聚合失败；不能因某一步报错跳过其他资源，也不能在旧资源存活时开始新一代
连接。

## 13. 日志与隐私

2026-09-14 元素导航的可选 `diagnostic_sink/diagnostic_enabled` 由 RC003 接入当前
writer；独立 OrthoFocus 可省略，两者仍共用导航源码。新增记录沿用原日志开关、
异步限量队列、轮转及导出，不创建新日志文件或配置字段。关闭详细诊断时不采集新增
前台句柄；启动/退出及 worker 错误仍沿用普通 app.log 兜底。

`key_received/key_route/key_finished/key_error` 只记录既有导航键，不记录字符输入，
包含当前回调的 `key_seq`、按下/松开、注入标志、扫描号、目标/前台句柄、导航状态、
分支及原回调返回值或异常类型/错误码。回调内只新增一次前台句柄标量读取，不进行
UIA、窗口文本或进程查询。`key_seq` 只关联一次钩子回调，不冒充 HID 手势号或异步
移动命令号；目标字段表示本次导航根窗口，前台句柄另存。返回值不证明目标软件已响应。

`collection_filter` 复用实际扫描和筛选结果，按 `collection_seq`、窗口句柄、标准
控件类型与原因汇总；每次收集最多 128 组，超出合并为 `summary_limit`。类型只允许
UIA 标准名称，未知值写 `UnknownControl`。`visited`、候选、排除和限深计数是不同
阶段，不能相加当作节点总数；总数以同次 `collection.visited` 为准。内部原本吞掉的
目标属性异常额外用 `target_read_error` 保留首个异常类型/错误码，不记录原异常正文。
不重新读取 UIA 属性，不记录名称、AutomationId、聊天/输入内容、截图或控件对象。
这些记录只增强排查，不改变候选过滤、按键路由、原生菜单放行或异常传播。

自身窗口修复新增 `local_input` 元数据，记录导航 VK、动作、入口原因
（`mapped_action` / `qt_key`）与入队结果，沿用原诊断开关。不记录窗口键盘文字；
动作执行与选中项变化仍由原 `action` / `move_result` 记录确认，入队不等于移动成功。

2026-09-13 设置页原“日志目录”入口改为“导出日志”。`log_export` 复用
`logging_setup` 与 `diagnostic_trace` 的文件名和轮转数量，只读取固定白名单，
不递归收集日志目录，也不跟随单个日志的链接/重解析文件。单文件最多 8 MiB，
从已打开文件大小取得有界快照；超长文本保留近期完整行。原日志与写入服务不变。
`export-info.json` 是 ZIP 内的衍生说明，记录程序版本、导出时间和文件读取状态，
不含配置、录音或原始本地路径；源文件仍在写入时不承诺跨文件原子快照。
UI 复用原 CompactLink，标准保存对话框确认目标，默认桌面及带日期时间的 ZIP 名；
后台任务复用 SettingsController 的线程登记、退出取消与迟到结果丢弃，重复点击
不启动并行压缩。临时 ZIP 位于所选目标目录，完成后原子替换选定文件，失败清理
自有临时文件并保留原目标。导出文件归用户、不自动上传或随程序分发，见
`CHECK-LOG-EXPORT-001`。既有只读日志位置辅助函数保留供兼容调用，已不是 UI 入口。

2026-09-13 通用程序上下文由 `voice_interaction_diagnostics_windows` 统一维护，
消费者与验收见 `CHECK-PROGRAM-LOG-001`。进程名按当前 PID 实时以只读权限查询，
仅保留 basename，不跨 PID 生命周期缓存。前台窗口前后不稳定时明确标记；HKL
仅为该前台线程的输入区域/布局标识，不冒充具体 TSF 档案。依据微软
[GetKeyboardLayout](https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-getkeyboardlayout)
的线程参数读取，现有 STA 输入法档案观察另保留其自身作用域。
诊断 writer 执行受限前台时间线，输入回调中不执行这类定时查询；没有用户输入
触发时不持续观察。语音开始后最多 30 秒，结束后另观察最多 10 秒，普通映射 2 秒、250 ms
采样并按变化输出；不保证捕获短于采样周期的窗口。

`input_environment_diagnostics_windows` 是两种诊断 writer 共用的只读采集来源，
不参与按键发送或语音运行决策。通过 QUERY_LIMITED_INFORMATION / TOKEN_QUERY
读取 observer、前台 PID 与已知语音进程名候选的提升状态、完整性 RID、UIAccess、
会话和创建时间；按前台身份变化或 5 秒间隔刷新并标记采样时刻，不能把缓存当成实时值。
候选名复用 `voice_program_manager` 原有 provider 清单，最多 32 项，核对实时 basename，
名称不符、权限拒绝和枚举失败分别保留未知，不把同名候选冒充通过模块验证的服务。
按微软 [GetTokenInformation](https://learn.microsoft.com/en-us/windows/win32/api/securitybaseapi/nf-securitybaseapi-gettokeninformation)
读取查询结果，不请求更高权限；完整性 SID 先验证缓冲区边界，仅输出末级 RID。
剪贴板仅调用 [GetClipboardSequenceNumber](https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-getclipboardsequencenumber)，
不打开剪贴板或读取文本，零值为不可用；序号变化不代表某输入法复制或已上屏。
`input_context_observation` 关联 attempt/provider，区分语音中与结束后观察；长录音
超过起始观察窗口后仍保留结束后的窗口，并发普通映射不覆盖它。独立接收入口的
前台/进程/剪贴板查询也在 writer 执行，钩子线程仅转交和排队；仍需在被控机显式
运行才能获得那台机器的接收证据。验收见 `CHECK-SUBMISSION-LOG-001`。

`shortcut_observation_windows` 是显式诊断入口而非第二个按键后端，独立进程由
`__main__` 在正常启动前分派。指定键与修饰键白名单、最长 120 秒及 2000 事件上限，
绝对新输出路径、异步写盘，任何键均按
[LowLevelKeyboardProc](https://learn.microsoft.com/en-us/previous-versions/windows/desktop/legacy/ms644985%28v%3Dvs.85%29)
约定继续传递，退出卸钩，不启动服务或调用发送/激活输入法入口。来源与目标响应
保持未知；注入标记不是 UU 或其它进程的身份证明。输出为可删除的本地衍生证据，
无网络传输、不参与配置或运行决策，采集端与主控端的事实不能互相冒充。


2026-09-13 补充：诊断事实仍由 `diagnostic_trace` 统一异步写入。开启既有诊断开关时，
`logs/diagnostic-report.json` 是可删除、不会用于运行决策的本地衍生证据，schema 1，
最多 1 MiB/最近 3 次故障；以最多 96 条前序事件及 10 秒后续事件组成，每次最多
192 条，30 秒内同会话故障合并，必要时裁剪。不采集音频、输入正文、标题、原始
设备路径；只保存现有安全事件和状态，不能保证每种未知故障都产生报告或直接给根因。
writer 采用临时文件替换，失败不改变设备/按键行为；关闭开关后保留已有文件。

宿主诊断扩展不改变协议 4、来源修订 1 或接管规则。ready 中脚本构建标识是填入
标识占位符前的脚本 SHA-256，区别于最终完整脚本文件摘要；两者各用于运行身份
和磁盘身份核验。未自报为未知，不改握手拒收条件。来源失败证据只读已验证对象的
注册表句柄层级及其 Enum 实例根身份，不改注册表、不关闭宿主持有的句柄；启用
诊断后每轮最多 8 个来源各查询一次，比较结果不作为设备接管依据。使用微软
[KEY_NAME_INFORMATION](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/ntddk/ns-ntddk-_key_name_information)
定义的长度读取键名，用户态按
[NtQueryKey 约定](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/wdm/nf-wdm-zwquerykey)
查询；名称只在内存分类，日志只保留固定分类、状态码和摘要短引用。

语音边沿确认与现有 attempt 关联，区分未建立钩子、原生发送异常、本地钩子确认
超时和成功；前台进程/线程只供判断焦点关联。确认成功仍不等于语音程序开麦、
音频到达或正确上屏。设备登记摘要和 profile 元数据可沿用至后续设备适配；当前
底层实现的实际支持范围没有因此扩大。

主要运行日志为 `%LOCALAPPDATA%\RemoteMic\RC003\logs\app.log`，5 MiB 轮转并保留
3 个备份。管理员按键组件安装/移除的最终结果由普通主进程脱敏后一次写入
`logs\hid-helper.log` 并立即关闭；高权限助手不写用户可控路径。日志允许记录：

- 固定状态名、异常类型；
- 逻辑按钮名、语音模式；
- PCM 帧数、时长、峰值等统计；
- 候选数量、布尔匹配和退出码。

日志禁止记录：

- 蓝牙地址；
- WinRT device id、HID/设备接口路径；
- 设备 token、原始 handle/address；
- 用户语音内容或识别文本；
- 原生异常中可能夹带的机器本地路径。

持久日志 handler 会移除 traceback，并把作为日志参数传入的异常替换为异常
类型。这只是第二道防线，调用点仍必须使用固定消息和隐私安全字段。

## 14. 构建、安装与第三方资产

`build/RemoteMicRC003.spec` 生成 one-dir、windowed、unsigned PyInstaller
候选。成品根目录只暴露普通权限桌面 EXE；自包含、普通 manifest 的窄职责 HID 助手
收在 `_internal` 内，不作为用户入口展示；
助手只打入固定任务生命周期、WUDFHost 核验/注入代码、固定 Gadget 资产及选择摘要的
受限读取。复用配置模块的正式路径，只读原 JSON 的选择字段，不调用配置归一化；构建
排除 Qt、BLE、音频、诊断扫描及语音设置依赖。桌面构建同时包含 QML、设备 profiles、遥控器图片
和已验证的可选资产。

`build/build-candidate.ps1` 的门禁顺序是：

1. 准备虚拟环境和固定依赖；
2. 公开边界扫描；
3. 下载并校验固定 VB-CABLE 包；
4. 下载并校验固定 Frida Gadget；
5. 运行带 `ResourceWarning` 门禁的完整测试；
6. PyInstaller 构建并确认根目录只有主 EXE、内部目录存在 HID 助手；
7. 冻结 EXE `--dry-run` 和 Qt 运行时检查。

Frida Gadget 与 VB-CABLE 都是候选构建的固定下载和哈希门禁；PyInstaller spec
还会再次校验 Frida，并把它分别放入桌面目录和自包含 HID 助手。缺失、哈希不符或
助手未生成都会直接停止构建，不能产出没有完整 HID 能力却沿用候选结论的包。

历史安装器源码 `installer/RemoteMicRC003Setup.iss` 为 per-user 安装，不再进入新发布封装；不自动启动、
不自动安装驱动。文件写入后只通过 Windows 管理员确认启动 `_internal` 中 HID 助手的固定
`--install-task`；
用户拒绝 UAC 时主程序仍可安装，但全部自定义按键映射停用，只保留 Windows 原始按键
操作，并提供“修复权限”。升级和卸载前
调用 `stop-app.ps1`，只处理安装目录下、完整路径、PID 与 CreationDate 都匹配的
`RemoteMicRC003.exe`。支持完整退出合同的版本只接受原子请求并等待正常清理；拒绝或
超时就停止安装，绝不强杀。已发布的旧双进程版本先通过旧托盘控制窗口正常停止桥接，
并校验窗口所属 PID，随后把剩余设置壳唤到前台，等待用户保存或放弃修改并完全退出；
安装器不强杀设置壳。只有明确权限不足时才
在管理员确认后重试一次；控制入口缺失或身份无法确认时直接停止，不弹无意义 UAC。确认进程
全部结束后，升级才删除并重写纯程序目录 `{app}\_internal`，并删除旧版曾留在根目录的
同名 HID 助手，不会删除 LocalAppData 中的
配置、日志或采集数据。卸载在删除程序文件前用固定 `--uninstall-task` 删除计划任务和
Program Files 助手；UAC 取消或清理失败会中止卸载，避免留下指向失效文件的任务。

Frida Gadget 与 VB-CABLE 包都使用固定 URL/version/SHA-256。运行时仍再次
校验，不把“构建时下载成功”当成永久可信。

## 15. 验证层级与当前事实

| 层级 | 能证明什么 | 不能证明什么 |
| --- | --- | --- |
| unittest | 状态机、异常路径、资源所有权、隐私/构建契约 | 真实蓝牙、驱动、输入法行为 |
| Windows API smoke | ctypes ABI、Raw Input/SendInput/托盘生命周期 | RC003 每个 usage 和长期稳定性 |
| QML 离屏加载 | QML 可创建、布局关键契约、窗口生命周期 | 用户机器显卡/缩放/主题全部组合 |
| 冻结 EXE smoke | PyInstaller 依赖完整、入口可运行 | BLE/音频/权限真机成功 |
| 真实硬件验收 | 当前候选在指定机器上的按键和语音事实 | 其他机器、休眠、长期、杀软兼容 |

2026-09-04 当前源码完成 1920 项完整 unittest，7 项按平台或安全条件跳过，测试日志
未出现 `ResourceWarning`、未关闭事件循环或 socket；`compileall`、源码 `--dry-run`、
公开边界扫描 508 个文件、`pip check` 和 `git diff --check` 均通过。本批没有构建、
打包或发布，因此这些结果证明当前源码的自动化基线，不证明冻结 EXE、安装器、计划任务
或真机链路已经通过。详细历史审查命令与证据见
`reviews/2026-08-21-full-code-audit.md`，后续批次见 `MAINTENANCE.md`。

截至 2026-08-30，历史异机结果已证明：多候选中可选出唯一可用 RC003、全部
普通按键可在映射页识别、记事本映射有效、按住语音得到非零 PCM 和识别文字、
F5 不再向输入框泄漏日期时间。On-request 真机探针最终未收到 `START_SEARCH`，
开关型已正式撤下。最新代码检查点还需要复测方向键单次触发与长按、HID 失联补松、
重复启动只唤出旧窗口、旧桥接升级迁移、重新登录无 UAC 自启、预授权助手修复/卸载，
以及 HOLD 短流尾音、断连重连、休眠恢复、长期运行、权限和杀软兼容。

## 16. 模块索引

| 领域 | 主要文件 |
| --- | --- |
| 入口与总装 | `launcher.py`、`__main__.py`、`app.py` |
| 连接监督 | `connection_supervisor.py`、`single_instance.py` |
| BLE/ATVV | `ble_transport_winrt.py`、`atvv_protocol.py`、`atvv_session.py` |
| 音频 | `audio_output.py`、`audio_playback.py`、`audio_playback_worker.py` |
| 普通按键 | `raw_input_windows.py`、`hid_identity.py`、`button_gesture.py` |
| 动作输出 | `key_mapping.py`、`win32_input.py`、`action_executor.py` |
| F5/跨来源语音仲裁 | `app.py`、`hotkey_capture_windows.py`、`voice_key_physicalizer_windows.py` |
| HID tap 与预授权助手 | `frida_compat.py`、`frida_hid_tap_runtime.py`、`frida_hid_tap_injector.py`、`hid_elevation_windows.py`、`hid_helper_launcher.py` |
| 豆包兼容 | `doubao_rpc.py` |
| 配置/IPC | `config.py`、`key_detection_bridge.py`、`key_testing.py` |
| 设置界面 | `qt_settings_app.py`、`settings_ui.py`、`qml/*.qml` |
| 手动更新 | `application_update.py`、`qt_settings_app.py`、`qml/DevicePage.qml`、`qml/main.qml` |
| 诊断/日志 | `windows_diagnostics.py`、`logging_setup.py`、顶层诊断脚本；历史 On-request 探针已撤下 |
| 资源/设备目录 | `resources.py`、`device_catalog.py`、`device-profiles/` |
| 驱动帮助 | `vb_cable_bundle.py` |
| 构建安装 | `build/`、`installer/`、Windows CI workflow |

## 17. 文档入口

| 想了解什么 | 权威入口 |
| --- | --- |
| 项目背景、构成原理、当前边界 | 本文 |
| 安装、配置、运行、测试和构建命令 | `README.md` |
| 上游归属、许可证和第三方边界 | `ATTRIBUTION.md`、仓库根目录许可文件 |
| 当前测试方法与人工验收项目 | `TESTING.md` |
| 性能基线、真实性能问题和优化路线 | `PERFORMANCE.md` |
| 单个已知故障的现象、原因和修复 | `bugs/BUG-*.md` |
| 维护批次、源码基线和验证证据 | `MAINTENANCE.md` |
| 用户可见版本变化 | `CHANGELOG.md` |
| 一次完整代码检查的范围和结论 | `reviews/` |
| 尚未实施的功能推演与过程讨论 | `discussions/` |

建议第一次接手时依次阅读本文、`README.md`、`TESTING.md`，再按当前任务读取
对应的 `bugs/`、维护记录或检查报告；不需要先通读所有历史流水。

## 18. 台账维护规则

- 新改动必须先确定资源所有者和失败后的重试边界。
- 输入路径不能在回调/钩子线程执行可能阻塞的 PortAudio、磁盘或进程操作。
- 控制事件不能与可丢弃音频共用一个会被音频填满的队列。
- `AUDIO_STOP` 不能越过应用已经收到的同代音频；跨队列处理必须保留顺序证据。
- 宿主快捷键与设备 `MIC_OPEN/MIC_CLOSE` 必须按既定先后顺序交付。
- 自动检查、冻结 smoke、真机验收必须分开写，不互相冒充。
- 候选的源码提交、目录 EXE 哈希、ZIP 哈希和 ZIP 内 EXE 复核写入维护/发布
  记录；本文只说明这一产物身份规则，不重复维护每个包的具体数值。
- 不提交二进制、设备身份、个人路径、凭据或语音内容。
- 对外分发继续满足 GPL-3.0-only 源码提供与第三方通知义务。
- 只有项目目标、用户工作流、模块职责、关键数据链、正式边界或长期有效结论
  发生变化时才更新本文；单次修复细节先进入故障、维护或检查记录。
