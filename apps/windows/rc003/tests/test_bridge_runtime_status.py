...
class BridgeRuntimeStatusTests(unittest.TestCase):
    ...
    def test_existing_schema_two_status_defaults_new_voice_runtime_fields(self):
        ...
        path.write_text(
            json.dumps(
                {
                    "schema": bridge_runtime_status.PREVIOUS_SCHEMA_VERSION,
                    "state": "connected",
                    "pid": 1234,
                    "updated_at": 42.0,
                }
        ...
        self.assertEqual(
            status.schema,
            bridge_runtime_status.PREVIOUS_SCHEMA_VERSION,
        )
        self.assertEqual(
            status.voice_runtime_state,
            bridge_runtime_status.VOICE_RUNTIME_NOT_TESTED,
        )
        self.assertEqual(status.voice_runtime_provider, "")
        self.assertIsNone(status.voice_runtime_updated_at)
    ...
    def test_retry_wait_state_round_trips_without_failure_details(self):
        written = bridge_runtime_status.publish_status(
            self.root,
        ...
        self.assertEqual(payload["state"], "retry_wait")
