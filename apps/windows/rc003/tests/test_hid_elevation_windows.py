...
class InstalledHelperTests(unittest.TestCase):
    ...
    def test_pre_reload_generation_nine_helper_requires_upgrade(self):
        self._assert_helper_generation_requires_upgrade(9)

    def test_protocol_three_generation_eight_helper_requires_upgrade(self):
        self._assert_helper_generation_requires_upgrade(8)

    def _assert_helper_generation_requires_upgrade(self, generation):
        # candidate.30 and candidate.31 shared generation 8 even though their
        # Gadget scripts used different interception protocols.
        with tempfile.TemporaryDirectory() as raw:
            root, app, owner_root, manifest_path, _target, manifest, task = (
                self._fixture(raw)
            )
            data = manifest.as_dict()
            data["helper_generation"] = generation
            data["helper_relative_path"] = hid_elevation_windows.helper_relative_path(
                manifest.helper_sha256, generation=generation
            ).as_posix()
            manifest_path.write_text(__import__("json").dumps(data), encoding="utf-8")
            state = hid_elevation_windows.inspect_installed_helper(
                frozen=True,
