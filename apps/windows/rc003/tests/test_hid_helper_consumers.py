...
class HidHelperConsumerTests(unittest.TestCase):
    ...
    def test_registration_prunes_stale_markers_and_preserves_unknown_markers(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            config_root = root / "config"
            stale = self._distribution(root, "stale", installed=False)
            stale_marker = hid_helper_consumers.register_current_consumer(
                config_root, frozen=True, executable=str(stale)
            )
            assert stale_marker is not None
            unknown_marker = stale_marker.with_name("unknown.json")
            unknown_marker.write_text("{broken", encoding="utf-8")
            shutil.rmtree(stale.parent)
            current = self._distribution(root, "current", installed=False)

            hid_helper_consumers.register_current_consumer(
                config_root, frozen=True, executable=str(current)
            )

            self.assertFalse(stale_marker.exists())
            self.assertTrue(unknown_marker.is_file())

    def test_portable_remove_is_global_when_another_copy_remains(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            config_root = root / "config"
            other = self._distribution(root, "candidate-24", installed=False)
            current = self._distribution(root, "candidate-25", installed=False)
            other_marker = hid_helper_consumers.register_current_consumer(
                config_root, frozen=True, executable=str(other)
            )
            ...
            remove_helper.assert_called_once_with()
            self.assertTrue(other_marker.is_file())
            self.assertFalse(current_marker.exists())

    def test_failed_global_portable_remove_restores_current_marker(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            config_root = root / "config"
            other = self._distribution(root, "candidate-24", installed=False)
            ...
            state = hid_helper_consumers.remove_for_portable_consumer(
                config_root,
                lambda: hid_elevation_windows.HidHelperState(False, "uac_cancelled"),
                frozen=True,
                executable=str(current),
            )
    ...
    def test_distribution_uninstall_keeps_helper_for_another_portable_copy(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            config_root = root / "config"
            installed = self._distribution(root, "installed", installed=True)
