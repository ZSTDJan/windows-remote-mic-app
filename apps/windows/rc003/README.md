...
- Remote Mic 连接页的“输出端点”（语音输出设备） → 优先选择
...
**一键随包安装（XRBM-031，可选）**：打开"设置 → 诊断"页，"可选：
...
安装并选择端点后，可以在“设置 → 诊断”的“VB-CABLE 通道”一行点击
“测试通道”。测试只在用户点击后运行，桥接运行时会拒绝启动；它会发送约一秒
...
打开设置后，“当前设备”只提供“小米蓝牙语音遥控器 2 Pro（RC003）”，继续使用
桥接、CABLE 输出、语音热键和 13 键映射。旧配置若曾保存 DJI Mic 2，本次运行会
...
1. 打开"设置"（Start Menu 中的"Remote Mic · RC003"或
   "Remote Mic · RC003 设置"），在"连接"页的"输出端点"下拉框中选择
   上一节配置好的端点；
2. 启动桥接有两种等价方式：在设置窗口"保存与启动"区域点击"保存并
   启动桥接"（会先用和"仅保存设置"完全相同的校验保存设置，校验通过后
   才启动桥接进程），或者关闭设置窗口后从 Start Menu 选择"启动
   Remote Mic · RC003"（它会以 `--bridge` 参数启动桥接）。“保存与启动”区域会
...
4. 在“连接”页填写唯一的“语音按键”，并让它与输入法自身设置
   ...
   且由 Windows 管理，不随桥接强行启动。选择“其他输入法或自定义程序”时可指定
   `.exe` 或 `.lnk`。搜狗和自定义程序可随桥接尝试启动，查找或启动失败不会阻止
   桥接。只有勾选“管理员启动”时才会出现 UAC；
...
1. 直接双击 `RemoteMicRC003.exe`（不带参数）即可打开设置窗口——这是默认
   ...
   直接双击，效果相同。在连接页“输出端点”下拉框中选择上一节配置好的端点；
2. 启动桥接有两种等价方式：在连接页“保存与启动”区域点击"保存并
   启动桥接"（先保存、校验通过后才启动）；或者关闭设置窗口，在同一个
   文件夹里运行 `.\RemoteMicRC003.exe --bridge` 启动桥接进程
   （不带参数会再次打开设置窗口，所以桥接必须显式用 `--bridge`）。“保存与启动”
   区域会显示未运行、正在启动、等待 RC003、RC003 已连接或启动失败等真实阶段；
...
开关型持续传声的最后一条协议探针随后也未收到启动信号，因此当前产品已正式
移除开关型运行路径，只保留历史调查证据。按住说话的新收尾选项仍待当前候选
复测。历史记录不能替代当前构建的验收；详见
`MAINTENANCE.md` 与 `TESTING.md`。只有用户明确确认后，程序才会通过 Windows 的
`runas`/UAC 启动 VB-Audio 官方安装器；Remote Mic 自身不会提权。
用户还可以在“连接 → 语音输入”中明确选择让 Windows 以管理员权限启动搜狗或
自定义第三方语音程序；微信输入法由 Windows 管理，不提供该选项。该操作不会提升
...
- 语音快捷键使用带私有标记的虚拟键 `keybd_event`。新配置默认发送右侧 Alt；已有配置继续保留原值。`DoubaoPhysicalizer` 只为右侧 Alt 配置处理豆包 `ImeService.exe` 的 injected 标记。通用三键仍可作为自定义快捷键被解析和发送，但 `Ctrl + Alt + F8` 经 UU 的按住时长不稳定，没有作为默认值发包。当前只保存一套按住说话快捷键，不把 Windows `Win+H` 当作豆包输入法的验收目标；UU 远程的按住边沿仍按 `TESTING.md` 做真机验收。
- “语音程序”是独立的可选管理层，默认关闭。当前内置搜狗语音组件和微信输入法自动发现，并支持用户指定其它 `.exe`/`.lnk`。微信输入法只检测安装和运行状态，由 Windows 管理；搜狗和自定义程序可随桥接尝试启动。普通启动沿用 Remote Mic 当前权限，管理员启动必须由用户勾选并通过 Windows UAC；程序缺失、UAC 取消或启动失败都不会阻止桥接。
- 提供 RC003 的 13 键统一映射界面。只有实体话筒键可选择“按住说话”，它也可以改为普通动作；其他实体键只提供普通动作或组合键。电源、返回、音量等特殊键仍在扫描码层抑制原生边沿后执行所选动作。
- 提供“连接”“按键”“权限”“诊断”四个设置页面；诊断页会区分“已检测到”和“需要手动验证”，不会把进程存活伪装成硬件验收通过。
...
1. 设置页保存设备、输出端点、按住说话快捷键和按键映射。保存时会校验设备类型、组合键、只有实体话筒键可传声的边界和次级动作边界，并对所选音频端点执行 open/start/stop/close 预检，再以原子写 + 回读校验落盘。桥接进程在下一次输入事件前热加载映射和语音设置；连接与输出端点变更仍在重启后应用。
