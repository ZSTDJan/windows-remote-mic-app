# Remote Mic — Windows client（小米遥控器2 Pro）
...
1. 打开程序（Start Menu 中的“Remote Mic · 小米遥控器2 Pro”或
   “Remote Mic · 小米遥控器2 Pro 设置”），先在“设备”页检查小米遥控器2 Pro 和按键接收，再到“语音”
...
4. 在“语音”页先选择语音程序。搜狗会优先读取自身当前的“按住说话”快捷键；微信
   ...
   语音助手运行时会拒绝改写，需先退出。微信输入法没有稳定的静默读写入口，Remote
   Mic 只保存自己的分程序记忆，不会自动打开微信设置、控制鼠标或代改微信快捷键；请用
   同一行右侧的“打开设置”进入微信设置并手动保持一致。搜狗已安装时不显示设置
   按钮，状态文字会提示在任务栏通知区域（含隐藏图标）右键搜狗语音图标并选择
   “设置”；Remote Mic 不会控制托盘、鼠标或设置窗口。未安装搜狗语音时，按钮显示
...
5. 需要完全结束程序时，右键通知区域图标选择“完全退出”，或从 Start Menu 选择
   “停止 Remote Mic · RC003”。完全退出会先正常清理 BLE、HID、语音热键与音频资源。
...
- 语音快捷键按目标程序选择注入方式：微信输入法先尝试状态栏语音按钮，失败时按言灵的已验证方式分别发送无私有标记、全虚拟键的 `SendInput` 按下与抬起批次，中间保持 80 ms；其它现有语音程序继续使用带私有标记的虚拟键 `keybd_event`。每个语音程序分别保存一套“按住说话”快捷键，不增加点按说话。搜狗切换时优先读取自身当前值，录入后先同步并读回；微信输入法和自定义程序只保存 Remote Mic 本地记忆，Windows 语音输入固定为 `Win+H`。UU 远程的按住边沿仍按 `TESTING.md` 做真机验收。
