...
`0.2.0-candidate.6`，唯一来源是 `src/ovb_rc003/VERSION`；程序窗口、冻结构建、
...
会让该启动项失效。便携版不会安装或卸载预授权 HID 助手；若电脑上没有同版本安装版
已经登记的助手，自定义方向映射需要手动以管理员权限启动便携版。普通权限启动时不会
连击，方向键只保留 Windows 原始操作。具体步骤见下一节“便携版 ZIP 用户”。
...
3. 按一下已映射为普通动作的按键确认映射生效。普通权限便携版在没有同版本预授权
   HID 助手时不执行自定义方向映射，方向键只保留 Windows 原始操作一次；确定等其它
   按键不受这一降级影响；
...
调用安装时已经授权的固定计划任务，不弹 UAC；任务中的助手会再次独立定位并核验
...
- 完整冻结包包含经过固定哈希校验的 Frida Gadget 和独立 HID 助手；安装版需在安装时
  确认一次 UAC，升级、修复或卸载该管理员组件时会再次确认。日常启动、桥接和登录自启
