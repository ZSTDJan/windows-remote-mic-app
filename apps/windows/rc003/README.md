...
这是本仓库独立维护的 Windows 客户端，用于把蓝牙语音遥控器的按键和语音桥接到
Windows。当前公开版本首先适配小米蓝牙语音遥控器 2 Pro（内部型号 RC003），
提供按键映射和 ATVV（Android TV Voice-over-BLE）语音桥接。项目整体说明请阅读
仓库根目录的 `README.md`。
...
>
> **运行无线麦时需要管理员权限。** 安装程序本身不需要管理员权限；安装完成后或
> 解压便携版后，请右键无线麦并选择“以管理员身份运行”。普通权限可以打开程序，
> 但返回、音量等依赖 HID tap 的实体按键可能无法完整识别。
...
为保证 RC003 的实体按键完整可用，安装后请右键开始菜单或桌面上的“无线麦”，选择
“以管理员身份运行”。
...
首次打开时请右键 `RemoteMicRC003.exe`，选择“以管理员身份运行”。

“随 Windows 启动”当前使用普通用户启动项，不会自动获得管理员权限。需要完整使用
返回、音量等按键时，请退出已经自动启动的普通权限实例，再以管理员身份打开无线麦；
也可以关闭“随 Windows 启动”，登录后手动以管理员身份运行。
...
(UAC) 提示启动官方原始的 `VBCABLE_Setup_x64.exe`——这个按钮不会改变无线麦自身
当前的运行权限，UAC 提示可以随时取消，取消不会安装任何内容。安装完成后需要重启
...
1. 右键 Start Menu 中的“无线麦”或桌面快捷方式，选择“以管理员身份运行”，
   先在“设备”页检查
...
1. 右键 `RemoteMicRC003.exe`，选择“以管理员身份运行”即可打开程序窗口；
   也可以在管理员 PowerShell 里运行 `.\RemoteMicRC003.exe --settings`。在“语音”
   页点击“应用”检测并选择 CABLE Input，再选择语音程序和语音按键；端点下拉、
   语音程序和语音按键会各自自动保存；
...
程序才会通过 Windows 的
`runas`/UAC 启动 VB-Audio 官方安装器；这个操作不会改变无线麦自身当前的运行权限。
...
- VB-CABLE 只是可选的语音路由方案。只有用户明确点击并确认 UAC 后，才会通过 `runas` 启动 VB-Audio 官方安装器；这个操作不会改变无线麦自身当前的运行权限，也不会修改系统默认输入/输出设备。
