...
`0.2.0-candidate.4`，唯一来源是 `src/ovb_rc003/VERSION`；程序窗口、冻结构建、
...
提示并正常退出，更早版本则会被唤到前台，等待用户从通知区域选择“完全退出”。旧版
完全退出后，同一次双击会继续打开当前便携版。程序不会强杀旧版；取消、超时、退出状态
或切换请求无法安全确认时，当前便携版不会启动。
