...
4. 在“语音”页先选择语音程序。搜狗会优先读取自身当前的“按住说话”快捷键；微信
   ...
   同一行右侧的“打开设置”进入微信设置并手动保持一致。搜狗已安装时，“打开设置”
   通过其托盘菜单进入独立的搜狗语音设置，不再打开搜狗输入法通用设置，也不移动
   鼠标；未安装搜狗语音时，按钮改为“去安装”，能找到 AI 工具箱就打开并在底部
   状态栏提示用户手动安装，找不到工具箱则只给出同一提示，不自动点击下载。
   搜狗、微信输入法和 Windows 语音输入都提供该设置按钮，自定义程序不显示。只有实体话筒键
   可以选择“按住说话”，其他按键只能选择普通动作或组合键。`Alt + Space` 经 UU
   远程会打开窗口左上角
...
- “语音程序”是独立的可选管理层，默认关闭。当前内置搜狗语音组件、微信输入法和 Windows 语音输入，并支持用户指定其它 `.exe`/`.lnk`。搜狗快捷键通过其自身落盘配置读取和原子改写，运行中拒绝改写；搜狗设置通过托盘控件打开独立设置窗口，未安装时转到 AI 工具箱并提示手动安装。微信输入法不再使用 UI Automation 读取或修改快捷键，设置页只由用户点击“打开设置”后正常打开。微信输入法和 Windows 语音输入由系统管理；搜狗和自定义程序可随遥控器服务尝试启动。配置 schema 8 按程序保存管理员启动偏好：新配置的搜狗默认开启、自定义程序默认关闭，旧的共享选择原样迁移，用户后续选择永久记忆。管理员启动通过 Windows UAC；程序缺失、UAC 取消或启动失败都不会阻止服务。
