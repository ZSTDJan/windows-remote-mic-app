...
> 测试；真实小米遥控器2 Pro 已验证多候选连接、13 个实体按键在
...
> 本候选已完成部分真实小米遥控器2 Pro 路径验证，仍有明确的真机复测队列；未签名，
...
`0.2.0-candidate.21`，唯一来源是 `src/ovb_rc003/VERSION`；程序窗口、冻结构建、
...
覆盖安装新版时，安装器在写入任何程序文件前先请求旧版完整退出。当前版本如果有未保存
的按键修改，会恢复旧窗口并让用户选择保存或放弃；旧的双进程版本会先正常停止遥控器
服务，再把可能已经隐藏的设置窗口唤到前台，提示用户从通知区域选择“完全退出”。安装器
不会强杀旧设置窗口，避免丢掉尚未保存的按键修改。无法确认安全退出、用户没有处理保存
...
2. 回到“设备”页点击“启动桥接”运行遥控器服务。“设备”页的“启动桥接”使用上次已经保存的
   ...
   选择“完全退出”。已有合法的“隐藏到通知区域”或“完全退出”选择继续保留；
...
4. 在“语音”页先选择语音程序。搜狗会优先读取自身当前的“按住说话”快捷键；微信
   ...
   默认值覆盖。
   点击语音按键输入框只录入“按住说话”快捷键。搜狗会先同步并读回，成功后才保存本地
   配置；语音助手运行时会拒绝改写，需先退出。微信输入法没有稳定的静默读写入口，
   无线麦只保存自己的分程序记忆，不会自动打开微信设置、控制鼠标或代改微信快捷键；
   请用同一行右侧的“打开设置”进入微信设置，并让微信语音界面的按住型快捷键与无线麦
   的“语音按键”一致。搜狗已安装时不显示设置
...
5. 需要完全结束程序时，右键通知区域图标选择“完全退出”，或从 Start Menu 选择
   “停止 无线麦”。两个入口都会先正常清理 BLE、HID、语音热键与音频资源；有未保存
   的按键修改时会恢复窗口，让用户直接选择保存并退出、不保存或取消。
   ...
   日志。**卸载不会自动删除设置和日志**：`config.json`、`key_bindings.json`、`logs\app.log`
...
5. **停止**：右键 Windows 右下角通知区域的无线麦图标，选择
   ...
   **卸载/移除**：如果这台电脑以后不再使用任何无线麦版本，先在“设备”页
   “按键接收”一行点击“移除权限”并确认一次 UAC，再删除整个解压文件夹。若其它
   安装版或便携版仍在使用，程序会拒绝移除共享助手。直接删除解压文件夹只会移除
   程序本体，不会自动删除已授权助手。但便携版运行时同样会把 `config.json`、
...
松开、横向滚动或一键双击。任意单击、双击、长按或遥控器组合都可以选择这些动作。
“元素导航开关”也可用于上述四类入口：首次触发扫描当前前台窗口，再次触发退出当前
导航。导航运行时仍使用桌面
...
保存新映射时，已经开始的单击、双击、长按、连发或遥控器组合会继续使用保存前的动作，
...
出来的设备报告并执行一次映射，不靠通用低层键盘钩子猜测设备身份。方向键不再接受
迟到的 Raw Input 映射：tap 正常时完整保留单击、组合和长按映射，tap 异常时只让
Windows 原始方向通过一次，不再产生原始方向加映射方向的双触发；tap 从可用状态失联
时会先为已经按下的按钮补齐松键，再切回 Raw Input 降级，避免方向或映射状态卡住。全部
...
遥控器点按持续传声的最后一条协议探针随后也未收到启动信号，因此当前产品只保留
   实体话筒键按住说话。宿主侧全部只执行按住型快捷键：微信输入法使用无私有标记的
   `SendInput` 发送真实按下和抬起，搜狗等程序使用原有按住型注入；不再点击微信状态栏
   语音按钮，也不再发送两次短点模拟开关。微信与其它按住型程序仍待当前源码真机复测。
   历史记录
不能替代当前构建的验收；详见
...
- 对 Windows Raw Input 丢失或到达过晚的 HID usages，复用上游的 Frida Gadget WUDFHost tap。该 tap 上报遥控器的**全部键盘 usage**（返回 `0xF1`、音量 `0x80/0x81`、方向/OK/Home/Menu/TV/Power 等），并在已核验的 RC003 `WUDFHost.exe` 内复制后清空原报告；Python 端只处理复制报告并注入一次映射动作。安装版和便携版首次启用时通过一次 UAC 登记固定的按需任务，普通权限主程序只启动该任务；Windows 回读任务时允许补出一个直属、无属性、无内容的空 `Triggers` 容器，但任何真实触发器、自动维护或失败重启设置都会被拒绝。管理员助手自行重新定位并核验 RC003 的 `WUDFHost.exe`，不接收 PID、程序路径或用户配置。助手异常时全部自定义按键映射停用，遥控器只保留 Windows 原始按键操作；运行中失联会先补齐已认领按键的松键。
...
- 语音快捷键按目标程序选择注入方式：微信输入法使用无私有标记、全虚拟键的 `SendInput` 发送真实按下与抬起；其它现有语音程序继续使用带私有标记的虚拟键 `keybd_event`。所有程序都只保存并执行一套“按住说话”快捷键，开关型旧配置不参与运行。搜狗切换时优先读取自身当前值，录入后先同步并读回；微信输入法和自定义程序只保存无线麦本地记忆，Windows 语音输入固定为 `Win+H`。UU 远程的按住边沿仍按 `TESTING.md` 做真机验收。
- “语音程序”是独立的可选管理层，默认关闭。当前内置搜狗语音组件、微信输入法和 Windows 语音输入，并支持用户指定其它 `.exe`/`.lnk`。搜狗快捷键通过其自身落盘配置读取和原子改写，运行中拒绝改写；已安装时界面只提示用户从通知区域右键搜狗语音图标进入设置，不控制托盘、鼠标或设置窗口，未安装时“去安装”可转到 AI 工具箱并提示手动安装。微信输入法不再使用 UI Automation 读取或修改快捷键，设置页只由用户点击“打开设置”后正常打开。微信输入法和 Windows 语音输入由系统管理；搜狗和自定义程序可随遥控器服务尝试启动。当前配置 schema 12 对新配置、缺失值和非法值默认使用“隐藏到通知区域”，已有合法的“隐藏到通知区域”或“完全退出”选择原样保留。新配置的搜狗默认开启管理员启动、自定义程序默认关闭，旧的共享选择原样迁移，用户后续选择永久记忆。管理员启动通过 Windows UAC；程序缺失、UAC 取消或启动失败都不会阻止服务。
- 提供 RC003 的 13 键统一映射界面，以及一层受限的遥控器组合映射。普通动作和组合动作均可使用内置键盘动作、鼠标单击/滚轮、电脑快捷键或 `quicker:runaction:` URI。只有实体话筒键可选择“按住说话”，它也可以改为普通动作；其他实体键只提供普通动作。HID tap 可用时由 Gadget 在设备栈内认领原报告后执行所选动作；不可用时全部自定义按键映射停用，只保留 Windows 原始按键操作。
...
1. “语音”页的输出端点、语音程序和按住说话快捷键各自自动保存；搜狗切换时读取真实快捷键，录入后同步并读回，微信输入法和自定义程序保存各自的本地记忆，Windows 语音输入固定为 `Win+H`。快捷键会按当前语音程序的支持范围校验，不合规时保留原值并说明原因。第一行“应用”只负责重新检测 VB-CABLE 并选择可用的 CABLE Input。“按键”页单独保存单键、手势和遥控器组合映射。保存时会校验电脑组合键、Quicker URI、组合主键冲突、只有实体话筒键可传声的边界和次级动作边界；音频端点选择会执行 open/start/stop/close 预检，再以原子写 + 回读校验落盘。桥接 worker 在下一次输入事件前热加载映射和语音设置；输出端点变更仍在重启后应用。
...
3. RC003 HID tap 在配对的 WUDFHost 内读取全部键盘 usage，把方向/OK/Home/Menu/TV/Power/返回/音量边沿送入同一映射层。Gadget 在目标进程内复制并清空被接管的原报告，Python socket 线程接收复制报告并执行一次动作。安装版由固定计划任务按需启动 Program Files 中的窄职责管理员助手；主程序不提权，自启动也不触发 UAC。方向键只接受 tap 事件执行映射；tap 失败时 Raw Input 不再补发映射，Windows 原始方向键照常通过一次；运行中失联先串行补齐已经按下的边沿，再启用 Raw Input 降级。实体话筒键按住说话时同时驱动宿主快捷键和 ATVV 音频，其他按键只进入普通动作分发。
4. 元素导航运行对象在 Qt 桌面主进程启动时创建；按键回调只提交开关和目标窗口，实际
