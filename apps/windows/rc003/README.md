...
- 桌面程序使用 PySide6 Essentials + Qt Quick/QML；便携版和安装器都通过 PyInstaller 打包，不要求终端用户另装 Python 或 Qt。用户入口仍只有 `RemoteMicRC003.exe`：双击（无参数）或 `--settings` 打开同一个桌面程序，登录自启使用隐藏参数 `--background`，`--bridge` 只保留为兼容启动形式，仍会进入同一个桌面主进程并请求启动进程内桥接 worker。成品目录另带 `RemoteMicRC003HidHelper.exe`，只供安装器、“修复权限”和固定计划任务调用，是按需结束的窄职责助手，不是第二套主程序。同一 Windows 登录会话只保留一个长期主进程；再次启动只恢复已有窗口，兼容 `--bridge` 则只通知已有进程启动服务。
