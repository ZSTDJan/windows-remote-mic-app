...
实体话筒键按住说话。宿主侧按程序适配：微信输入法在按下和音频停止时各短点一次
快捷键，搜狗等按住型程序仍发送按下和抬起。微信专用协议与其它按住型程序仍待当前
测试包真机复测。历史记录不能替代当前构建的验收；详见
