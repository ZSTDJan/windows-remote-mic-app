...
> 主程序、桥接和随 Windows 启动始终保持普通权限。VB-CABLE 仍是需要用户明确安装的
> 可选第三方组件（见下文）。
...
HID tap 在独立线程上报事件，低层钩子吞掉对应原生键并只注入映射动作。方向键不再
接受迟到的 Raw Input 映射：tap 正常时完整保留单击、组合和长按映射，tap 异常时只让
Windows 原始方向通过一次，不再产生原始方向加映射方向的双触发；全部
...
- 对 Windows Raw Input 丢失的 HID usages，复用上游的 Frida Gadget WUDFHost tap。该 tap 上报遥控器的**全部键盘 usage**（返回 `0xF1`、音量 `0x80/0x81`、方向/OK/Home/Menu/TV/Power 等），作为所有普通按键的输入旁路：它在独立 socket 线程上 arm，低层键盘钩子零等待匹配并吞掉原生键，只注入一次映射动作。安装版通过一次 UAC 登记固定的按需任务，普通权限桥接只启动该任务；管理员助手自行重新定位并核验 RC003 的 `WUDFHost.exe`，不接收 PID、程序路径或用户配置。助手异常时方向映射停用，Windows 原始方向键保留一次。
...
- 桌面程序使用 PySide6 Essentials + Qt Quick/QML；便携版和安装器都通过 PyInstaller 打包，不要求终端用户另装 Python 或 Qt。用户入口仍只有 `RemoteMicRC003.exe`：双击（无参数）或 `--settings` 打开同一个桌面程序，登录自启使用隐藏参数 `--background`，`--bridge` 保留为独立桥接兼容入口。成品目录另带 `RemoteMicRC003HidHelper.exe`，只供安装器和“修复权限”调用，不是第二套主程序。同一 Windows 登录会话只保留一个程序窗口；再次双击程序、任务栏或通知区域入口时，只恢复并置前已有窗口，不再新开一份。
...
3. RC003 HID tap 在配对的 WUDFHost 内读取全部键盘 usage，把方向/OK/Home/Menu/TV/Power/返回/音量边沿送入同一映射层，并在独立 socket 线程上 arm，低层钩子零等待吞掉原生键后只注入一次动作。安装版由固定计划任务按需启动 Program Files 中的窄职责管理员助手；主程序和桥接不提权，自启动也不触发 UAC。方向键只接受 tap 事件执行映射；tap 失败时 Raw Input 不再补发映射，Windows 原始方向键照常通过一次。实体话筒键按住说话时同时驱动宿主快捷键和 ATVV 音频，其他按键只进入普通动作分发。
