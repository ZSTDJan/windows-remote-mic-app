...
> 改为由按键主映射选择开关型或按住型语音，并允许麦克风键改做普通动作；
> 非麦克风键启动持续 ATVV 语音、两种生命周期的完整区分、重连、休眠和长期
> 运行仍待 `fix15` 真机复测。
...
2. 启动桥接有两种等价方式：在设置窗口底部"桥接控制"区域点击"保存并
   ...
   **不代表已经与 RC003 建立连接**，实际连接、按键和语音状态仍以下一步
   的日志为准。桥接运行后，Windows 右下角通知区域会出现 Remote Mic
   图标；双击图标打开设置，右键可选择"打开设置"或"退出桥接"。关闭
   设置窗口只关闭设置，不会停止桥接；
3. 按一下已映射为普通动作的按键（例如方向键、确定键）确认按键映射生效；
4. 在“按键映射”页分别填写“开关型语音快捷键”和“按住型语音快捷键”，并让
   它们与豆包输入法自身设置一致；再把需要的一个实体按键主映射为对应语音动作。
   默认可以继续使用麦克风键，也可以改用其他键；同一时间最多允许一个语音主映射。
   新安装的豆包输入法预设为切换模式 `ralt+space`、按住模式 `ralt`，不要把
   Windows `Win+H` 当作豆包快捷键。然后先手动确认对应组合键本身能正常工作：
   打开记事本（或任意可编辑文本框），把光标点进文本区域，用实体键盘按下组合键，
   确认豆包语音功能出现并能输入文字。豆包的麦克风输入设备也必须选择
   `CABLE Output`（如果按上一节配置了 VB-CABLE）。手动测试通过后，光标保持在
   同一个可编辑文本框中，再使用已映射的遥控器语音按键说话；如果手动组合键都无法
...
RC003 共 **13 个可映射物理按键**。麦克风图标键默认承担开关型语音动作，但也
可以改为普通动作；其他按键也可在主映射中选择“开关型语音”或“按住型语音”。
同一时间最多允许一个语音主映射，双击和长按不允许语音动作。遥控器**没有独立
的物理静音键**（“系统静音”只是可选的手动绑定，不是任何按键的默认映射）；
“返回”默认映射为退格动作。设置页的“检测真实按键”同时使用 Raw Input 与可选
HID tap；后者补齐 Windows 普通输入链路丢失的返回和音量键。

普通按键已有的双击/长按设置在主映射临时改为语音时会保留，但语音期间不执行；
把主映射切回普通动作后，这些次级设置会重新显示并继续生效。
...
以及非麦克风键触发持续 ATVV 语音，是 `fix15` 必须单独完成的真机门槛，不能由
自动测试或既有麦克风键语音结果代替。
...
但 `fix14` 的切换续开回声保护、`fix15` 的统一按键语音动作，以及 HOLD/TOGGLE
完整区分仍待当前候选复测。历史记录不能替代当前构建的验收；详见
...
- 语音快捷键使用带私有标记的虚拟键 `keybd_event`；`DoubaoPhysicalizer` 附加到豆包 `ImeService.exe` 的低层回调，只对该标记事件清除 `LLKHF_INJECTED` / lower-integrity 标志并清空 `dwExtraInfo`，再把右 Alt 事件转交给后续钩子，因此豆包看到的形状与实体右 Alt 一致。两套快捷键分别保存；实际生命周期由唯一语音主映射中的“开关型语音”或“按住型语音”决定，不把 Windows `Win+H` 当作豆包输入法的验收目标。
- 提供 RC003 的 13 键统一映射界面。任一实体键的主映射可选择普通动作或两种语音动作，麦克风键不再强制绑定语音；电源、返回、音量等特殊键仍在扫描码层抑制原生边沿后执行所选动作。
...
- 配置写入采用临时文件 + fsync + `os.replace` 原子替换，失败不会留下半写的 JSON；桥接进程在下一次按键或语音事件前按 mtime 热加载新保存的按键映射、显式语音动作和两套快捷键，活跃会话会在收尾后再切换，磁盘数据损坏时保留最后一份有效设置。连接设备和音频输出端点仍需重启桥接。
...
1. 设置页保存设备、输出端点、两套语音快捷键和按键映射；语音生命周期由唯一语音主映射的动作类型决定。保存时会校验设备类型、组合键、语音主映射唯一性和次级动作边界，并对所选音频端点执行 open/start/stop/close 预检，再以原子写 + 回读校验落盘。桥接进程在下一次输入事件前热加载映射和语音设置；连接与输出端点变更仍在重启后应用。
2. 桥接进程启动单实例保护和通知区域控制图标，并由连接监督器负责 BLE
   ...
   陈旧缓存。
3. 可选的 RC003 HID tap 在配对的 WUDFHost 内读取全部键盘 usage，把方向/OK/Home/Menu/TV/Power/返回/音量边沿送入同一映射层，并在独立 socket 线程上 arm，低层钩子零等待吞掉原生键后只注入一次动作；按键主映射决定走普通动作分发还是语音状态机，语音动作再交付对应快捷键并启动/停止 ATVV 音频流。
