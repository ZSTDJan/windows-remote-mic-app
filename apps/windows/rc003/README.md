...
4. 在“语音”页先选择语音程序。Remote Mic 会优先读取搜狗或微信输入法当前的
   “按住说话”快捷键；读取不到时才使用该程序上次记住的值。每个程序分别记忆，
   初始值为搜狗右侧 Ctrl、微信输入法左侧 Ctrl + 左侧 Win、Windows 语音输入
   `Win+H`、自定义程序右侧 Alt。旧配置中原来保存的全局快捷键会迁移给当时选中的
   程序，不会被新默认值覆盖。
   点击语音按键输入框重新录入后，Remote Mic 会先同步到目标程序并读回确认，成功后
   才保存本地配置；任一步失败都会恢复原值。搜狗语音助手运行时会拒绝改写，需先
   退出搜狗语音助手；微信输入法会临时打开自身设置页完成读取或修改。只有实体话筒键
...
- 语音快捷键按目标程序选择注入方式：微信输入法先尝试状态栏语音按钮，失败时按言灵的已验证方式分别发送无私有标记、全虚拟键的 `SendInput` 按下与抬起批次，中间保持 80 ms；其它现有语音程序继续使用带私有标记的虚拟键 `keybd_event`。每个语音程序分别保存一套“按住说话”快捷键，不增加点按说话。切换搜狗或微信输入法时优先读取程序当前值；录入后先同步第三方并读回，再原子保存 Remote Mic 配置。Windows 语音输入固定为 `Win+H`，自定义程序只保存 Remote Mic 本地值。UU 远程的按住边沿仍按 `TESTING.md` 做真机验收。
- “语音程序”是独立的可选管理层，默认关闭。当前内置搜狗语音组件、微信输入法和 Windows 语音输入，并支持用户指定其它 `.exe`/`.lnk`。搜狗快捷键通过其自身落盘配置读取和原子改写，运行中拒绝改写；微信输入法通过自身设置窗口的 Windows UI Automation 语义控件读取和修改，不写私有 MMKV、不使用 OCR 或固定屏幕坐标。微信输入法和 Windows 语音输入由系统管理；搜狗和自定义程序可随遥控器服务尝试启动。普通启动沿用 Remote Mic 当前权限，管理员启动必须由用户勾选并通过 Windows UAC；程序缺失、UAC 取消或启动失败都不会阻止服务。
...
1. “语音”页的输出端点、语音程序和按住说话快捷键各自自动保存；切换程序时优先读取该程序真实快捷键，录入后先同步第三方并读回确认，再保存本地的分程序记忆。第一行“应用”只负责重新检测 VB-CABLE 并选择可用的 CABLE Input。“按键”页单独保存单键、手势和遥控器组合映射。保存时会校验电脑组合键、Quicker URI、组合主键冲突、只有实体话筒键可传声的边界和次级动作边界；音频端点选择会执行 open/start/stop/close 预检，再以原子写 + 回读校验落盘。桥接进程在下一次输入事件前热加载映射和语音设置；输出端点变更仍在重启后应用。
