...
> 哈希的 Frida Gadget 和独立 HID 管理员助手；安装版只在安装该助手时请求一次 UAC，
> 主程序和随 Windows 启动始终保持普通权限。每个登录会话只有一个长期主进程，遥控器
> 服务和元素导航都由该进程持有。VB-CABLE 仍是需要用户明确安装的
> 可选第三方组件（见下文）。
...
- 完整冻结包包含经过固定哈希校验的 Frida Gadget 和独立 HID 助手；安装版需在安装时
  ...
  不弹 UAC。源码调试仍需从管理员终端启动；只有日志出现 tap ready 和真实按键边沿，
  才能确认 HID 旁路可用。
