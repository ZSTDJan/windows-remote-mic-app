...
`0.2.0-candidate.10`，唯一来源是 `src/ovb_rc003/VERSION`；程序窗口、冻结构建、
...
2. 回到“设备”页点击“启动桥接”运行遥控器服务。“设备”页的“启动桥接”使用上次已经保存的
   ...
   单击或双击通知区域图标会恢复同一个任务栏窗口。关闭窗口默认会先正常停止遥控器服务
   再退出程序；需要后台运行时，可在“设备”页底部改为“隐藏到通知区域”。从旧配置首次
   升级到本版时，原来的默认隐藏值会一次性迁移为完全退出；之后用户主动选择隐藏会保留；
...
1. 直接双击 `RemoteMicRC003.exe`（不带参数）即可打开程序窗口——这是默认
   ...
   首次启用自定义按键映射时，按简短提示确认并在 Windows UAC 中选择“是”。成功后以后普通
   双击和随 Windows 启动都不再为自定义按键映射重复弹 UAC；
2. 在“设备”页点击“启动桥接”运行遥控器服务（使用上次已经保存的设置，不保存其它未应用
   ...
   连接分别显示灰色斜杠麦克风、灰色麦克风和蓝色麦克风。关闭窗口默认会先正常停止
   遥控器服务并完全退出；需要后台运行时，可在“设备”页改为隐藏到通知区域；
...
- Qt 桌面主进程统一持有窗口、通知区域图标、桥接 worker 和元素导航。图标按服务未启动、等待小米遥控器2 Pro、小米遥控器2 Pro 已连接显示灰色斜杠麦克风、灰色麦克风和蓝色麦克风。关闭窗口默认先正常停止桥接并完全退出；用户主动选择隐藏到通知区域时才保留后台运行，退出失败则恢复窗口并提示。
