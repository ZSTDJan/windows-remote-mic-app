...
“常规”页明确开启“随 Windows 启动”。Start Menu 分组提供程序主入口、设置入口、
停止和卸载；主入口与桌面快捷方式打开同一个桌面程序，任务栏图标和窗口一一对应。
是否同时自动启动遥控器服务，由“启动程序时运行遥控器服务”选项决定。
...
或卸载程序；仍可在程序“常规”页明确开启随 Windows 启动，但之后移动或删除解压目录
...
打开程序后，“设备”页的当前设备只提供“小米蓝牙语音遥控器 2 Pro（RC003）”，
“语音”页维护 CABLE 输出、语音程序和语音按键，“按键”页维护 13 键映射，“常规”页
维护开机启动、自动运行遥控器服务和关闭窗口行为。旧配置
...
1. 直接双击 `RemoteMicRC003.exe`（不带参数）即可打开程序窗口——这是默认
   ...
   页点击“应用”检测并选择 CABLE Input，再选择语音程序和语音按键；端点下拉、
   语音程序和语音按键会各自自动保存；
2. 在“设备”页点击“启动”运行遥控器服务（使用上次已经保存的设置，不保存其它未应用
   修改）；也可以在“常规”页设置程序打开后自动运行服务，或与“随 Windows 启动”组合
   使用。`.\RemoteMicRC003.exe --bridge` 启动桥接仍作为独立兼容入口保留，正常桌面
   使用不需要执行它。“遥控器服务”
   一行会显示未运行、正在启动、等待 RC003、RC003 已连接或启动失败等真实阶段；
   只有桥接运行状态确认设备连接后，界面才显示“RC003 已连接”。
   Windows 右下角通知区域图标和窗口属于同一个程序；服务未启动、等待 RC003、已经
...
- 提供“设备”“按键”“语音”“常规”四个页面。设备与权限检查结果放回对应业务行，仍区分自动检查和“需要手动验证”，不会把进程存活伪装成硬件验收通过；常规页负责随 Windows 启动、程序启动时运行遥控器服务和关闭窗口行为。
...
- Qt 桌面程序统一持有任务栏窗口和通知区域图标；由它启动的桥接子进程不再显示第二个图标。图标按服务未启动、等待 RC003、RC003 已连接显示灰色斜杠麦克风、灰色麦克风和蓝色麦克风。关闭窗口默认隐藏到通知区域；选择完全退出时先正常停止桥接，失败则恢复窗口并提示。
