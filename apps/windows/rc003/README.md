...
安装并选择端点后，可以在“设置 → 检查与修复”的“VB-CABLE 通道”一行点击
...
4. 在“按键”页填写唯一的“语音快捷键”，并让它与输入法自身设置
   ...
   自身的快捷键或语音模式；
   同一行的“语音程序”按钮是可选管理入口：默认“不管理”，不会改变现有
   快捷键链路。选择搜狗后会自动查找已安装的搜狗语音组件；选择“自定义程序”
   时可指定 `.exe` 或 `.lnk`。可以手动启动，也可以明确选择随桥接启动；查找或
   启动失败不会阻止桥接。只有勾选“以管理员权限启动”时才会出现 UAC，取消后
   Remote Mic 继续正常运行；
...
用户还可以在“按键 → 语音程序”中明确选择让 Windows 以管理员权限启动已安装的
...
- “语音程序”是独立的可选管理层，默认关闭。当前内置搜狗语音组件自动发现，并支持用户指定其它 `.exe`/`.lnk`；可手动启动或随桥接启动。普通启动沿用 Remote Mic 当前权限，管理员启动必须由用户勾选并通过 Windows UAC；程序缺失、UAC 取消或启动失败都不会阻止桥接。
