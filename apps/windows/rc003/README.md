...
- “语音程序”是独立的可选管理层，默认关闭。当前内置搜狗语音组件、微信输入法和 Windows 语音输入，并支持用户指定其它 `.exe`/`.lnk`。搜狗快捷键通过其自身落盘配置读取和原子改写，运行中拒绝改写；已安装时界面只提示用户从通知区域右键搜狗语音图标进入设置，不控制托盘、鼠标或设置窗口，未安装时“去安装”可转到 AI 工具箱并提示手动安装。微信输入法不再使用 UI Automation 读取或修改快捷键，设置页只由用户点击“打开设置”后正常打开。微信输入法和 Windows 语音输入由系统管理；搜狗和自定义程序可随遥控器服务尝试启动。配置 schema 8 按程序保存管理员启动偏好：新配置的搜狗默认开启、自定义程序默认关闭，旧的共享选择原样迁移，用户后续选择永久记忆。管理员启动通过 Windows UAC；程序缺失、UAC 取消或启动失败都不会阻止服务。
