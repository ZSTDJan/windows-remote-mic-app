...
“修复权限”重试。安装完成后可以选择打开程序；需要登录后自动运行时，由用户在
...
服务，再清理已经不持有硬件资源的设置壳。无法确认安全退出、用户没有处理保存提示或
退出超时时，安装会直接停止，不会强杀后继续覆盖。只有旧程序本身以管理员身份运行、
...
- Qt 桌面主进程统一持有窗口、通知区域图标、桥接 worker 和元素导航。图标按服务未启动、等待小米遥控器2 Pro、小米遥控器2 Pro 已连接显示灰色斜杠麦克风、灰色麦克风和蓝色麦克风。关闭窗口默认隐藏到通知区域；选择完全退出时先正常停止桥接，失败则恢复窗口并提示。
