...
4. 在“连接”页填写唯一的“语音按键”，并让它与输入法自身设置
   ...
   选择搜狗后会自动查找已安装的搜狗语音组件；选择“其他输入法或自定义程序”
   时可指定 `.exe` 或 `.lnk`。选择具体程序后会随桥接尝试启动，查找或启动失败
   不会阻止桥接。只有勾选“管理员启动”时才会出现 UAC，取消后 Remote Mic
   继续正常运行；
...
用户还可以在“连接 → 语音输入”中明确选择让 Windows 以管理员权限启动已安装的
第三方语音程序；该操作不会提升 Remote Mic 自身，也不会把第三方程序打包进本项目。
...
- “语音程序”是独立的可选管理层，默认关闭。当前内置搜狗语音组件自动发现，并支持用户指定其它 `.exe`/`.lnk`；选择具体程序后随桥接尝试启动。普通启动沿用 Remote Mic 当前权限，管理员启动必须由用户勾选并通过 Windows UAC；程序缺失、UAC 取消或启动失败都不会阻止桥接。
