...
- 语音快捷键按目标程序选择注入方式：微信输入法先尝试状态栏语音按钮，失败时按言灵的已验证方式分别发送无私有标记、全虚拟键的 `SendInput` 按下与抬起批次，中间保持 80 ms；其它现有语音程序继续使用带私有标记的虚拟键 `keybd_event`。每个语音程序分别保存一套“按住说话”快捷键，不增加点按说话。搜狗切换时优先读取自身当前值，录入后先同步并读回；微信输入法和自定义程序只保存无线麦本地记忆，Windows 语音输入固定为 `Win+H`。UU 远程的按住边沿仍按 `TESTING.md` 做真机验收。
