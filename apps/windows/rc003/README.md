...
`0.2.0-candidate.8`，唯一来源是 `src/ovb_rc003/VERSION`；程序窗口、冻结构建、
...
方向键仍由 Windows 原样执行一次，但自定义方向映射会停用，设置页“按键接收”行可点
...
会让该启动项失效。首次启用方向改键时，便携版会先询问，再通过一次 UAC 安装或更新
共享的预授权 HID 助手；成功后主程序、日常启动和随 Windows 启动都保持普通权限，
不再反复弹 UAC。取消或失败时方向键只保留 Windows 原始操作一次，可在“设备”页
...
3. 按一下已映射为普通动作的按键确认映射生效。方向键只有在管理员按键组件正常时
   执行自定义映射；组件异常时不会双触发，只保留 Windows 原始方向键一次；
...
1. 直接双击 `RemoteMicRC003.exe`（不带参数）即可打开程序窗口——这是默认
   ...
   首次启用方向改键时，按简短提示确认并在 Windows UAC 中选择“是”。成功后以后普通
   双击和随 Windows 启动都不再为方向改键重复弹 UAC；
...
3. 按一下已映射为普通动作的按键确认映射生效。预授权 HID 助手可用时，普通权限
   便携版会执行保存的自定义方向映射；助手未启用或异常时，方向键只保留 Windows
   原始操作一次，确定等其它按键不受这一降级影响；
...
- 提供 RC003 的 13 键统一映射界面，以及一层受限的遥控器组合映射。普通动作和组合动作均可使用内置键盘动作、鼠标单击/滚轮、电脑快捷键或 `quicker:runaction:` URI。只有实体话筒键可选择“按住说话”，它也可以改为普通动作；其他实体键只提供普通动作。HID tap 可用时由 Gadget 在设备栈内认领原报告后执行所选动作；不可用时按各按键的明确降级规则处理。
...
- 完整冻结包包含经过固定哈希校验的 Frida Gadget 和独立 HID 助手；安装版或便携版
  首次启用方向改键时确认一次 UAC，升级、修复或卸载该管理员组件时会再次确认。日常启动、桥接和登录自启
