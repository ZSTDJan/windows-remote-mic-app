...
`0.2.0-candidate.24`，唯一来源是 `src/ovb_rc003/VERSION`；程序窗口、冻结构建、
...
4. 在“语音”页先选择语音程序。搜狗会优先读取自身当前的“按住说话”快捷键，自定义
   程序使用上次记住的值，Windows 语音输入固定使用 `Win+H`；微信输入法由无线麦直接
   控制语音按钮，不需要设置快捷键。旧配置中的微信快捷键只保留兼容读取，不再录入、
   校验或执行。
   搜狗和自定义程序可以点击“语音按键”输入框录入按住型快捷键；搜狗会先同步并读回，
   成功后才保存本地配置，语音助手运行时会拒绝改写，需先退出。微信输入法的输入框固定
   显示“无需设置”，同一行右侧的“打开设置”只用于打开微信自身设置，不是语音使用前置
   条件。搜狗已安装时不显示设置
   ...
   其它输入法的快捷键。自动读写不能证明快捷键未被其它软件抢占，因此搜狗和自定义
   ...
   豆包输入法变得可用。微信输入法不做这项快捷键测试：按下遥控器话筒键时直接打开
   微信语音，松开时直接结束并提交；
...
实体话筒键按住说话。微信输入法 `2.1.3.18` 的本机核实表明其快捷键不接受本程序可用的
Windows 模拟按键路径，因此微信改为按下时点击自身语音按钮、松开时再次点击结束；搜狗、
Windows 语音输入和自定义程序继续使用各自的按住型快捷键。微信快捷键旧配置不再生效。
历史记录
不能替代当前构建的验收；详见
...
- 语音入口按目标程序分开处理：微信输入法直接控制其状态栏语音按钮，并按同进程的语音窗口确认打开和收尾；不再发送、录入或校验微信快捷键。其它现有语音程序继续使用带私有标记的虚拟键 `keybd_event` 执行按住型快捷键，开关型旧配置不参与运行。搜狗切换时优先读取自身当前值，录入后先同步并读回；自定义程序保存无线麦本地记忆，Windows 语音输入固定为 `Win+H`。UU 远程的按住边沿仍按 `TESTING.md` 做真机验收。
...
1. “语音”页的输出端点和语音程序自动保存；搜狗切换时读取真实快捷键，录入后同步并读回，自定义程序保存本地记忆，Windows 语音输入固定为 `Win+H`，微信输入法无需快捷键。需要快捷键的程序会按各自支持范围校验，不合规时保留原值并说明原因。第一行“应用”只负责重新检测 VB-CABLE 并选择可用的 CABLE Input。“按键”页单独保存单键、手势和遥控器组合映射。保存时会校验电脑组合键、Quicker URI、组合主键冲突、只有实体话筒键可传声的边界和次级动作边界；音频端点选择会执行 open/start/stop/close 预检，再以原子写 + 回读校验落盘。桥接 worker 在下一次输入事件前热加载映射和语音设置；输出端点变更仍在重启后应用。
