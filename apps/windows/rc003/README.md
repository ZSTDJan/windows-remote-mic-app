...
开关”：首次触发扫描当前前台窗口，再次触发退出当前导航。导航运行时仍使用桌面
...
- 提供 RC003 的 13 键统一映射界面，以及一层受限的遥控器组合映射。普通动作和组合动作均可使用内置动作、电脑快捷键或 `quicker:runaction:` URI。只有实体话筒键可选择“按住说话”，它也可以改为普通动作；其他实体键只提供普通动作。HID tap 可用时由 Gadget 在设备栈内认领原报告后执行所选动作；不可用时按各按键的明确降级规则处理。
