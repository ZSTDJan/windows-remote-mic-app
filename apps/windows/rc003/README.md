...
1. “语音”页的输出端点、语音程序和按住说话快捷键各自自动保存；搜狗切换时读取真实快捷键，录入后同步并读回，微信输入法和自定义程序保存各自的本地记忆，Windows 语音输入固定为 `Win+H`。第一行“应用”只负责重新检测 VB-CABLE 并选择可用的 CABLE Input。“按键”页单独保存单键、手势和遥控器组合映射。保存时会校验电脑组合键、Quicker URI、组合主键冲突、只有实体话筒键可传声的边界和次级动作边界；音频端点选择会执行 open/start/stop/close 预检，再以原子写 + 回读校验落盘。桥接 worker 在下一次输入事件前热加载映射和语音设置；输出端点变更仍在重启后应用。
